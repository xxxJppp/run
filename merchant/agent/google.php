...
if (isset($_POST['onecode'])) {
    ...
    if ($checkResult) {
        ...
        $result = $mysql->update('ysk_merchant',['google_auth' => $google],'id='.$agent_info['id']);
        if ($result) {
            echo '设置成功';die;
        } else {
            echo '服務器錯誤';die;
...
if (isset($_POST['is_open_google_auth'])) {
    ...
    $result = $mysql->update('ysk_merchant',['is_open_google_auth' => $is_open_google_auth],'id='.$agent_info['id']);
    echo '设置成功';die;
}
