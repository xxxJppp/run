...
class UserController extends CommonController
{
    public function index()
    {
        ...
        $zfb = M('userrob')->where($map)->sum('price');
        $zfb = 0 + $zfb;
        $map2['uid'] = $uid;
        $map2['class'] = 1;
        $map2['status'] = 3;
        $map2['pipeitime'] = array(between, array($start, $end));
        $wx = M('userrob')->where($map2)->sum('price');
        $wx = 0 + $wx;
        $mapt['uid'] = $uid;
        $mapt['class'] = 3;
        $mapt['status'] = 3;
        $mapt['pipeitime'] = array(between, array($start, $end));
        $yhk = M('userrob')->where($mapt)->sum('price');
        $yhk = 0 + $yhk;
        $map3['uid'] = $uid;
        $map3['status'] = 3;
        $map3['pipeitime'] = array(between, array($start, $end));
        $zzz = M('userrob')->where($map3)->sum('price');
        $zzz = 0 + $zzz;
        $map4['uid'] = $uid;
        $map4['ts'] = array('gt', 0);
        $ts = M('userrob')->where($map4)->count();
        $ts = 0 + $ts;
    ...
    public function saveuma()
    {
        ...
        if (IS_POST) {
            ...
            if ($result) {
                ajaxReturn("修改成功", 1, '/User/erweima');
                exit;
            } else {
                ajaxReturn("修改上传失败");
                exit;
