...
class huafei
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_huafei_automatic_account', request::filter('get.page'), 10, $where, null, 'id', 'asc');
    ...
    public function startAutomaticRb()
    {
        ...
        $id = intval(request::filter('get.id'));
        //检查该微信
        $find_huafei = $this->mysql->query("client_huafei_automatic_account", "id={$id}")[0];
     //   if (!is_array($find_huafei)) functions::json(-3, '更改异常!');
        $training = 2;
        if ($find_huafei['training'] == 2) {
            //开启状态
            $training = 1;
            //检测账号是否异常
           // if ($find_huafei['status'] != 4) functions::json(-3, '更改失败,当前微信没有在线!');
        }
        $update = $this->mysql->update("client_huafei_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function orderCount()
    {
        ...
        $huafei_id = request::filter('get.huafei_id', '', 'trim');
    ...
    public function startAutomaticLogOut()
    {
        ...
        //检查该微信
        $find_huafei = $this->mysql->query("client_huafei_automatic_account", "id={$id}")[0];
        if (!is_array($find_huafei)) functions::json(-3, '当前微信出现异常!');
        if ($find_huafei['status'] == 6 || $find_huafei['status'] == 1) functions::json(-3, '当前微信账号已经安全注销过了!');
        $update = $this->mysql->update("client_huafei_automatic_account", [
            'status' => 1
        ], "id={$id}");
    ...
    public function automaticDelete()
    {
        ...
        //检查该微信
        $find_huafei = $this->mysql->query("client_huafei_automatic_account", "id={$id}")[0];
     //   if (!is_array($find_huafei)) functions::json(-2, '删除该微信号时出现一个错误!');
       // if ($find_huafei['status'] == 6) functions::json(-2, '当前微信正在进行安全注销,请耐心等待注销完成后再进行删除!');
       // if ($find_huafei['status'] != 1) functions::json(-2, '请将微信安全下线后再进行删除!');
        $this->mysql->delete("client_huafei_automatic_account", "id={$id}");
        ...
        if ($sorting == 'user') {
            if (!empty($code)) {
                if ($_GET['locking'] == 'true') {
                    $_SESSION['huafei']['WHERE'] = 'user_id=' . $code . ' ';
                }
            }
            if ($_GET['locking'] == 'false') {
                unset($_SESSION['huafei']['WHERE']);
            }
        }

        $where = $_SESSION['huafei']['WHERE'];
    ...
    public function callback()
    {
        ...
        $order = $this->mysql->query('client_huafei_automatic_orders', "id={$order_id}")[0];
    ...
    public function automaticOrderDelete()
    {
        $this->powerLogin(23);
        $id = intval(request::filter('get.id'));
        $this->mysql->delete("client_huafei_automatic_orders", "id={$id}");
