...
class shouqianba
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_shouqianba_automatic_account', request::filter('get.page'), 10, $where, null, 'id', 'asc');
    ...
    public function startAutomaticRb()
    {
        ...
        $id = intval(request::filter('get.id'));
        //检查该收钱吧
        $find_shouqianba = $this->mysql->query("client_shouqianba_automatic_account", "id={$id}")[0];
    //    if (!is_array($find_shouqianba)) functions::json(-3, '更改异常!');
        $training = 2;
        if ($find_shouqianba['training'] == 2) {
            //开启状态
            $training = 1;
            //检测账号是否异常
        //    if ($find_shouqianba['status'] != 4) functions::json(-3, '更改失败,当前收钱吧没有在线!');
        }
        $update = $this->mysql->update("client_shouqianba_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function startAutomaticGateway()
    {
      ...
      //  if (!is_array($find_shouqianba)) functions::json(-3, '更改异常!');
        $receiving = 2;
        if ($find_shouqianba['receiving'] == 2) {
            //开启状态
            $receiving = 1;
            //检测账号是否异常
          //  if ($find_shouqianba['status'] != 4) functions::json(-3, '更改失败,当前收钱吧没有在线!');
    ...
    public function startAutomaticLogOut()
    {
        ...
        //检查该收钱吧
        $find_shouqianba = $this->mysql->query("client_shouqianba_automatic_account", "id={$id}")[0];
        if (!is_array($find_shouqianba)) functions::json(-3, '当前收钱吧出现异常!');
        if ($find_shouqianba['status'] == 6 || $find_shouqianba['status'] == 1) functions::json(-3, '当前收钱吧账号已经安全注销过了!');
        $update = $this->mysql->update("client_shouqianba_automatic_account", [
            'status' => 1
        ], "id={$id}");
    ...
    public function automaticDelete()
    {
        ...
        //检查该收钱吧
        $find_shouqianba = $this->mysql->query("client_shouqianba_automatic_account", "id={$id}")[0];
      //  if (!is_array($find_shouqianba)) functions::json(-2, '删除该收钱吧号时出现一个错误!');
       // if ($find_shouqianba['status'] == 6) functions::json(-2, '当前收钱吧正在进行安全注销,请耐心等待注销完成后再进行删除!');
      //  if ($find_shouqianba['status'] != 1) functions::json(-2, '请将收钱吧安全下线后再进行删除!');
        $this->mysql->delete("client_shouqianba_automatic_account", "id={$id}");
    ...
    public function orderCount()
    {
        ...
        $shouqianba_id = request::filter('get.shouqianba_id', '', 'trim');
        ...
        if (!empty($shouqianba_id)) {
            $where .= " AND shouqianba_id='{$shouqianba_id}'";
        }
    ...
    public function automaticOrder()
    {
        ...
        if ($sorting == 'user') {
            if (!empty($code)) {
                if ($_GET['locking'] == 'true') {
                    $_SESSION['shouqianba']['WHERE'] = 'user_id=' . $code . ' ';
                }
            }
            if ($_GET['locking'] == 'false') {
                unset($_SESSION['shouqianba']['WHERE']);
            }
        }
        ...
        if ($sorting == 'shouqianba') {
            if ($code != '') {
                $code = intval($code);
                $_SESSION['shouqianba']['WHERE'] = "shouqianba_id={$code}";
            } else {
                unset($_SESSION['shouqianba']['WHERE']);
            }
        }

        $where = $_SESSION['shouqianba']['WHERE'];
    ...
    public function callback()
    {
        ...
        $order = $this->mysql->query('client_shouqianba_automatic_orders', "id={$order_id}")[0];
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $this->mysql->update("client_shouqianba_automatic_orders", [
                'pay_time' => time(),
                'status'   => 4
            ], "id={$order['id']}");
        }
    ...
    public function automaticOrderDelete()
    {
        $this->powerLogin(25);
        $id = intval(request::filter('get.id'));
        $this->mysql->delete("client_shouqianba_automatic_orders", "id={$id}");
    ...
    public function export() {
        ...
        $mysql = new Mysql();
        $list = $mysql->query('client_shouqianba_automatic_orders',$where);
