...
class wechatphone
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_wechatphone_automatic_account', request::filter('get.page'), 10, $where, null, 'id', 'asc');
    ...
    public function startAutomaticRb()
    {
        ...
        $id = intval(request::filter('get.id'));
        //检查该微信
        $find_wechatphone = $this->mysql->query("client_wechatphone_automatic_account", "id={$id}")[0];
     //   if (!is_array($find_wechatphone)) functions::json(-3, '更改异常!');
        $training = 2;
        if ($find_wechatphone['training'] == 2) {
            //开启状态
            $training = 1;
            //检测账号是否异常
           // if ($find_wechatphone['status'] != 4) functions::json(-3, '更改失败,当前微信没有在线!');
        }
        $update = $this->mysql->update("client_wechatphone_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function orderCount()
    {
        ...
        $wechatphone_id = request::filter('get.wechatphone_id', '', 'trim');
        ...
        if (!empty($wechatphone_id)) {
            $where .= " AND wechatphone_id='{$wechatphone_id}'";
        }
    ...
    public function startAutomaticLogOut()
    {
        ...
        //检查该微信
        $find_wechatphone = $this->mysql->query("client_wechatphone_automatic_account", "id={$id}")[0];
        if (!is_array($find_wechatphone)) functions::json(-3, '当前微信出现异常!');
    ...
    public function automaticDelete()
    {
        ...
        //检查该微信
        $find_wechatphone = $this->mysql->query("client_wechatphone_automatic_account", "id={$id}")[0];
     //   if (!is_array($find_wechatphone)) functions::json(-2, '删除该微信号时出现一个错误!');
       ...
       // if ($find_wechatphone['status'] != 1) functions::json(-2, '请将微信安全下线后再进行删除!');
        $this->mysql->delete("client_wechatphone_automatic_account", "id={$id}");
        ...
        if ($sorting == 'user') {
            if (!empty($code)) {
                if ($_GET['locking'] == 'true') {
                    $_SESSION['wechatphone']['WHERE'] = 'user_id=' . $code . ' ';
                }
            }
            if ($_GET['locking'] == 'false') {
                unset($_SESSION['wechatphone']['WHERE']);
            }
        }

        $where = $_SESSION['wechatphone']['WHERE'];
    ...
    public function callback()
    {
        ...
        $order = $this->mysql->query('client_wechatphone_automatic_orders', "id={$order_id}")[0];
    ...
    public function automaticOrderDelete()
    {
        $this->powerLogin(23);
        $id = intval(request::filter('get.id'));
        $this->mysql->delete("client_wechatphone_automatic_orders", "id={$id}");
