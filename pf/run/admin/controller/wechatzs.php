...
class wechatzs
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_wechatzs_automatic_account', request::filter('get.page'), 10, $where, null, 'id', 'asc');
    ...
    public function startAutomaticRb()
    {
        ...
        $id = intval(request::filter('get.id'));
        //检查该微信
        $find_wechatzs = $this->mysql->query("client_wechatzs_automatic_account", "id={$id}")[0];
     //   if (!is_array($find_wechatzs)) functions::json(-3, '更改异常!');
        $training = 2;
        if ($find_wechatzs['training'] == 2) {
            //开启状态
            $training = 1;
            //检测账号是否异常
           // if ($find_wechatzs['status'] != 4) functions::json(-3, '更改失败,当前微信没有在线!');
        }
        $update = $this->mysql->update("client_wechatzs_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function orderCount()
    {
        ...
        $wechatzs_id = request::filter('get.wechatzs_id', '', 'trim');
        ...
        if (!empty($wechatzs_id)) {
            $where .= " AND wechatzs_id='{$wechatzs_id}'";
        }
    ...
    public function startAutomaticLogOut()
    {
        ...
        //检查该微信
        $find_wechatzs = $this->mysql->query("client_wechatzs_automatic_account", "id={$id}")[0];
        if (!is_array($find_wechatzs)) functions::json(-3, '当前微信出现异常!');
    ...
    public function automaticDelete()
    {
        ...
        //检查该微信
        $find_wechatzs = $this->mysql->query("client_wechatzs_automatic_account", "id={$id}")[0];
     //   if (!is_array($find_wechatzs)) functions::json(-2, '删除该微信号时出现一个错误!');
       ...
       // if ($find_wechatzs['status'] != 1) functions::json(-2, '请将微信安全下线后再进行删除!');
        $this->mysql->delete("client_wechatzs_automatic_account", "id={$id}");
        ...
        if ($sorting == 'user') {
            if (!empty($code)) {
                if ($_GET['locking'] == 'true') {
                    $_SESSION['wechatzs']['WHERE'] = 'user_id=' . $code . ' ';
                }
            }
            if ($_GET['locking'] == 'false') {
                unset($_SESSION['wechatzs']['WHERE']);
            }
        }

        $where = $_SESSION['wechatzs']['WHERE'];
    ...
    public function callback()
    {
        ...
        $order = $this->mysql->query('client_wechatzs_automatic_orders', "id={$order_id}")[0];
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $this->mysql->update("client_wechatzs_automatic_orders", [
                'pay_time' => time(),
                'status'   => 4
            ], "id={$order['id']}");
        }
    ...
    public function automaticOrderDelete()
    {
        $this->powerLogin(23);
        $id = intval(request::filter('get.id'));
        $this->mysql->delete("client_wechatzs_automatic_orders", "id={$id}");
