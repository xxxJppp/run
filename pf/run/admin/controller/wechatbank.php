...
class wechatbank
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_wechatbank_automatic_account', request::filter('get.page'), 10, $where, null, 'id', 'asc');
    ...
    public function startAutomaticRb()
    {
        ...
        $id = intval(request::filter('get.id'));
        //检查该支付宝
        $find_alipay = $this->mysql->query("client_wechatbank_automatic_account", "id={$id}")[0];
      //  if (!is_array($find_alipay)) functions::json(-3, '更改异常!');
        ...
        $update = $this->mysql->update("client_wechatbank_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function automaticDelete()
    {
     ...
     //   if (!is_array($find_alipay)) functions::json(-2, '删除该支付宝号时出现一个错误!');
       ...
       // if ($find_alipay['status'] != 1) functions::json(-2, '请将支付宝安全下线后再进行删除!');
        $this->mysql->delete("client_wechatbank_automatic_account", "id={$id}");
    ...
    public function callback()
    {
        ...
        $order = $this->mysql->query('client_wechatbank_automatic_orders', "id={$order_id}")[0];
    ...
    public function automaticOrderDelete()
    {
        $this->powerLogin(25);
        $id = intval(request::filter('get.id'));
        $this->mysql->delete("client_wechatbank_automatic_orders", "id={$id}");
    ...
    public function export() {
        ...
        $mysql = new Mysql();
        $list = $mysql->query('client_wechatbank_automatic_orders',$where);
