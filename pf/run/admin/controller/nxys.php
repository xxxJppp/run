...
class nxys
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_nxys_automatic_account', request::filter('get.page'), 10, $where, null, 'id', 'asc');
    ...
    public function startAutomaticRb()
    {
        ...
        $id = intval(request::filter('get.id'));
        //检查该农信易扫
        $find_alipay = $this->mysql->query("client_nxys_automatic_account", "id={$id}")[0];
        ...
        if ($find_alipay['training'] == 2) {
            ...
            $training = 1;
            //检测账号是否异常
           // if ($find_alipay['status'] != 4) functions::json(-3, '更改失败,当前农信易扫没有在线!');
        }
        $update = $this->mysql->update("client_nxys_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function startAutomaticLogOut()
    {
        ...
        //检查该农信易扫
        $find_alipay = $this->mysql->query("client_nxys_automatic_account", "id={$id}")[0];
        if (!is_array($find_alipay)) functions::json(-3, '当前农信易扫出现异常!');
        if ($find_alipay['status'] == 6 || $find_alipay['status'] == 1) functions::json(-3, '当前农信易扫账号已经安全注销过了!');
        $update = $this->mysql->update("client_nxys_automatic_account", [
            'status' => 1
        ], "id={$id}");
    ...
    public function automaticDelete()
    {
        ...
        //检查该农信易扫
        $find_alipay = $this->mysql->query("client_nxys_automatic_account", "id={$id}")[0];
       // if (!is_array($find_alipay)) functions::json(-2, '删除该农信易扫号时出现一个错误!');
      //  if ($find_alipay['status'] == 6) functions::json(-2, '当前农信易扫正在进行安全注销,请耐心等待注销完成后再进行删除!');
      //  if ($find_alipay['status'] != 1) functions::json(-2, '请将农信易扫安全下线后再进行删除!');
        $this->mysql->delete("client_nxys_automatic_account", "id={$id}");
        functions::json(200, '您成功的删除了该农信易扫!');
    }
    //订单统计
    //权限ID：33
    public function orderCount()
    {
        ...
        $nxys_id = request::filter('get.nxys_id', '', 'trim');
    ...
    public function automaticOrder()
    {
        ...
        if ($sorting == 'alipay') {
            if ($code != '') {
                $code = intval($code);
                $_SESSION['ALIPAY']['WHERE'] = "nxys_id={$code}";
            } else {
                unset($_SESSION['ALIPAY']['WHERE']);
            }
        }
    ...
    public function callback()
    {
        ...
        $order = $this->mysql->query('client_nxys_automatic_orders', "id={$order_id}")[0];
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $this->mysql->update("client_nxys_automatic_orders", [
                'pay_time' => time(),
                'status'   => 4
            ], "id={$order['id']}");
        }
    ...
    public function automaticOrderDelete()
    {
        $this->powerLogin(25);
        $id = intval(request::filter('get.id'));
        $this->mysql->delete("client_nxys_automatic_orders", "id={$id}");
    ...
    public function export() {
        ...
        $mysql = new Mysql();
        $list = $mysql->query('client_nxys_automatic_orders',$where);
