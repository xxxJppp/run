      ...
      <div class="panel panel-default">
        <div class="panel-title">
          交易订单 [ <b>今日收入:</b> <?php //查询今日收入 
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_huafei_automatic_orders where {$where_call}");
          ...
          <table class="table table-hover">
            <thead>
              <tr>
                ...
                <th>支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status'){?>(<?php if ($sorting['code'] == 1) echo '获取订单中';if ($sorting['code'] == 2) echo '未支付';if ($sorting['code'] == 3) echo '订单超时';if ($sorting['code'] == 4) echo '已支付';?>)<?php }?><a href='<?php echo url::s('admin/huafei/automaticOrder',"sorting=status&code=".($sorting['code']+1));?>'> <i class="fa fa-unsorted"></i></a></th>
                <th>
                    <input onchange="member(this);" style="width: 30%;"  type="text" class="form-control-line" placeholder="商户ID" value="<?php if ($sorting['name'] == 'user') echo $_GET['code'];?>">  [ <a href="<?php echo url::s('admin/huafei/automaticOrder','sorting=user&code=&locking=false');?>">全部</a>  ]
                </th>
                <th>异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback'){?>(<?php if ($_GET['code'] == 0) echo '未回调';if ($_GET['code'] == 1) echo '已回调';?>)<?php }?><a href='<?php echo url::s('admin/huafei/automaticOrder',"sorting=callback&code=".($sorting['code']+1));?>'> <i class="fa fa-unsorted"></i></a></th>
                <th>回调信息</th>
            ...
            <?php  foreach ($result['result'] as $ru){?>
              <tr>
                <td>话费ID：<a href='<?php echo url::s("admin/huafei/automatic","id={$ru['huafei_id']}");?>'><?php echo $ru['huafei_id'];?></a> / 订单ID：<?php echo $ru['id'];?> ( <a target="_blank" href="<?php echo url::s('gateway/pay/automatichuafei',"id={$ru['id']}");?>">支付链接</a> )
                 ...
                 <td>订单号码：<?php echo $ru['trade_no'];?>
                  ...
                  <?php echo $ru['huafei_id'];?> |  <?php echo $ru['id'];?>
                ...
                <td>支付金额：<span style="color: green;"><b><?php echo $ru['amount'];?></b> <?php echo $ru['callback_status'] == 1 ? " ( 利: ". ($ru['amount']-$ru['fees']) ." )" : '';?></span>
                        ...
                        <td>商户信息：<?php $userInfo = $mysql->query("client_user","id={$ru['user_id']}")[0]; echo is_array($userInfo) ? '<a href="'. url::s("admin/huafei/automaticOrder","sorting=user&code={$userInfo[id]}&locking=true") .'"><span style="color:green;font-size:14px;font-weight:bold;">'.$userInfo['username'] .'</span></a>' . ' ( 商户ID: ' .  $userInfo['id']  . ' ) ' : '<span style="color:red;font-size:8px;">会员不存在</span>';?>
    ...
    <!-- End Panel -->
            ...
            function reissue(id){
            	  swal({   title: "订单通知",   
                      ...
                      function(){
                      //开始请求话费登录
                    	   $.get("<?php echo url::s('admin/huafei/automaticReissue',"id=");?>" + id, function(result){
                          	 if(result.code == '200'){
              ...
              function trade_no(obj){
                  location.href = "<?php echo url::s('admin/huafei/automaticOrder',"sorting=trade_no&code=");?>" + $(obj).val();
                  }
              
              function member(obj){
                  location.href = "<?php echo url::s('admin/huafei/automaticOrder',"sorting=user&locking=true&code=");?>" + $(obj).val();
                  }
              
              function huafei(){
                  var huafei = $('#huafei').val();
                  console.log(huafei);
                  location.href = "<?php echo url::s('admin/huafei/automaticOrder',"sorting=huafei&code=");?>" + huafei;
			...
			function del(id){
		              ...
		              function(){
		                 $.get("<?php echo url::s('admin/huafei/automaticOrderDelete','id=');?>" + id, function(result){
		           ...
		           swal({
		              ...
		              function(){
				           $("input[name='items']:checked").each(function(){
				        	 $.get("<?php echo url::s('admin/huafei/automaticOrderDelete','id=');?>" + $(this).val(), function(result){
			...
			function callback(){ 
		           swal({
		              ...
		              function(){
				           $("input[name='items']:checked").each(function(){
				        	 $.get("<?php echo url::s('admin/huafei/callback','id=');?>" + $(this).val(), function(result){
						            	swal("操作提示", '当前操作已经执行完毕!', "success");
						              	setTimeout(function(){location.href = '';},1000);
				               });
				           });  
