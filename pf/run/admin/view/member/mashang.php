    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        <div class="panel-title">
          码商管理<?php if (!empty($_GET['member_id'])){?> -> [ <a style="color:green;" href="<?php echo url::s("admin/member/mashang");?>">返回查看全部码商</a> ]<?php }?>
        </div>
        <div class="panel-body table-responsive">
          <table class="table table-hover">
            <thead>
              <tr>
                <td></td>
                <td><input onchange="user_query(this);" style="width: 80%;"  type="text" class="form-control form-control-line" placeholder="手机号/会员名/码商ID" value="<?php echo $_GET['member_id'];?>"></td>
                <td>账户详细</td>
                <td>今日获利</td>
                <td>昨日获利</td>
                <td>总获利</td>
            ...
            <?php  foreach ($member['result'] as $em){?>
              <tr>
               ...
               <td>
                ...
                <td>
                       ...
                       $nowTime = strtotime(date("Y-m-d",time()) . ' 00:00:00');
                        ...
                        $order = $mysql->select("select sum(fees) as fees,count(id) as count,sum(amount) as amount from {$fix}client_paofen_automatic_orders where user_id = {$em['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4");
               ...
               <td>
                        ...
                        $order = $mysql->select("select sum(fees) as fees,count(id) as count,sum(amount) as amount from {$fix}client_paofen_automatic_orders where user_id = {$em['id']} and status=4");
