            ...
            <?php  foreach ($result['result'] as $ru){?>
              <tr>
                ...
                <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and creation_time > {$nowTime} and status=4");
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4");
                        echo '<span style="color:red;font-weight:bold;"> '.floatval($order[0]['money']) .' </span> / 手续费: <span style="color:blue;">'. number_format($order[0]['fees'],3) .'</span>  ( 订单数量: <span style="color:green;font-weight:bold;">'.$order[0]['count'].'</span> )';
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and status=4");
                        echo '<span style="color:red;font-weight:bold;"> '.floatval($order[0]['money']) .' </span> / 手续费: <span style="color:blue;">'. number_format($order[0]['fees'],3) .'</span>  ( 订单数量: <span style="color:green;font-weight:bold;">'.$order[0]['count'].'</span> )';
                        ?>
                        </td>
                ...
                <p><a href="#" onclick="del('<?php echo $ru['id'];?>');" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i>移除微信</a></p>
                </td>
              </tr>
            <?php }?>
            </tbody>
    ...
    <!-- End Panel -->  
            ...
            function startAutomaticRb(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/wechat/startAutomaticRb',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            ...
            function startAutomaticGateway(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/wechat/startAutomaticGateway',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("微信提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
            ...
            function startAutomaticLogOut(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/wechat/startAutomaticLogOut',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("微信提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
			...
			function del(id){
		              swal({
		                ...
		                showCancelButton: true, 
		                confirmButtonColor: "#DD6B55", 
		                confirmButtonText: "是的,我要删除该微信!", 
		                closeOnConfirm: false 
		              },
		              function(){
		                 $.get("<?php echo url::s('admin/wechat/automaticDelete','id=');?>" + id, function(result){
			...
			function deletes(){ 
		           swal({
		                title: "非常危险", 
		                text: "你确定要批量删除已选中的微信吗？", 
		                type: "warning", 
		                showCancelButton: true, 
		                confirmButtonColor: "#DD6B55", 
		                confirmButtonText: "是的,我要删除这些微信!", 
		                closeOnConfirm: false 
		              },
		              function(){
				           $("input[name='items']:checked").each(function(){
				        	 $.get("<?php echo url::s('admin/wechat/automaticDelete','id=');?>" + $(this).val(), function(result){
