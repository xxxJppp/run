    ...
    font-size: 14px;" onclick="exportDoc()">导出表格</button>
                            ...
                            <tr>
                                <form action="" method="get">
                                    ...
                                    <th>账号ID：<input type="text" name="alipay_id" value="<?php if(!empty($_GET['alipay_id'])){  echo $_GET['alipay_id']; }?>"></th>
                            ...
                            <?php foreach ($result['result'] as $ru) { ?>
                                <tr>
                                    <td>账号ID：<a
                                                href='<?php echo url::s("admin/yunshanfu/automatic", "id={$ru['alipay_id']}"); ?>'><?php echo $ru['alipay_id']; ?></a>
                         ...
                         <?php echo $ru['alipay_id']; ?> | <?php echo $ru['id']; ?>
                                    ...
                                    <td>商户信息：<?php $userInfo = $mysql->query("client_user", "id={$ru['user_id']}")[0];
                                        echo is_array($userInfo) ? '<a href="' . url::s("admin/yunshanfu/automaticOrder", "sorting=user&code={$userInfo[id]}&locking=true") . '"><span style="color:green;font-size:14px;font-weight:bold;">' . $userInfo['username'] . '</span></a>' . ' ( 商户ID: ' . $userInfo['id'] . ' ) ' : '<span style="color:red;font-size:8px;">会员不存在</span>'; ?>
                                        <br>手机号码：<span
            ...
            <script type="text/javascript">
                ...
                function exportDoc () {
                    var alipay_id = GetQueryString('alipay_id');
                    var user_id = GetQueryString('user_id');
                    var status = GetQueryString('status');
                    var start_time = GetQueryString('start_time');
                    var end_time = GetQueryString('end_time');
                    location.href = "<?php echo url::s('admin/yunshanfu/export', "user_id=");?>" + user_id+ '&start_time='+start_time + '&end_time=' + end_time + '&status=' + status + 'alipay_id=' + alipay_id;
                }
