            ...
            <?php  foreach ($result['result'] as $ru){?>
              <tr>
               <td>
                    ...
                    <p>  ( <a href="<?php echo url::s('admin/nxys/automaticOrder',"sorting=alipay&code={$ru['id']}");?>">交易订单</a> )</p>
                    <p><b>APP登录用户名: </b> <?php echo $ru['app_user'];?></p>
                    <p><b>卡号：<?php echo $ru['account_no']; ?></p>
                ...
                <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_wechatbank_automatic_orders where alipay_id={$ru['id']} and creation_time > {$nowTime} and status=4");
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_wechatbank_automatic_orders where alipay_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4");
                        echo '<span style="color:red;font-weight:bold;"> '.floatval($order[0]['money']) .' </span> / 手续费: <span style="color:blue;">'. number_format($order[0]['fees'],3) .'</span>  ( 订单数量: <span style="color:green;font-weight:bold;">'.$order[0]['count'].'</span> )';
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_wechatbank_automatic_orders where alipay_id={$ru['id']} and status=4");
                        echo '<span style="color:red;font-weight:bold;"> '.floatval($order[0]['money']) .' </span> / 手续费: <span style="color:blue;">'. number_format($order[0]['fees'],3) .'</span>  ( 订单数量: <span style="color:green;font-weight:bold;">'.$order[0]['count'].'</span> )';
                        ?>
                        </td>
    ...
    <!-- End Panel -->  
            ...
            function startAutomaticRb(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/wechatbank/startAutomaticRb',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            ...
            function startAutomaticGateway(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/wechatbank/startAutomaticGateway',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("账号提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
            ...
            function startAutomaticLogOut(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/wechatbank/startAutomaticLogOut',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("账号提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
			...
			function del(id){
		              ...
		              function(){
		                 $.get("<?php echo url::s('admin/wechatbank/automaticDelete','id=');?>" + id, function(result){
		           ...
		           swal({
		              ...
		              function(){
				           $("input[name='items']:checked").each(function(){
				        	 $.get("<?php echo url::s('admin/wechatbank/automaticDelete','id=');?>" + $(this).val(), function(result){
