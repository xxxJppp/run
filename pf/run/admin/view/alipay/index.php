            ...
            <?php  foreach ($result['result'] as $ru){?>
              <tr>
                <td>
                    ...
                    <p><b>账户：<?php echo $ru['account']; ?></p>
                       <p><b>PID：<?php echo $ru['pid']; ?></p>
                </td>
                ...
                <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipay_automatic_orders where alipay_id={$ru['id']} and creation_time > {$nowTime} and status=4");
                        $total = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipay_automatic_orders where alipay_id={$ru['id']} and creation_time > {$nowTime}");
                        if($total[0]['count']){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipay_automatic_orders where alipay_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4");
                        $total = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipay_automatic_orders where alipay_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime}");
                            if($total[0]['count']){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_alipay_automatic_orders where alipay_id={$ru['id']} and status=4");
    ...
    <!-- End Panel -->  
            ...
            function startAutomaticRb(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/alipay/startAutomaticRb',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            ...
            function startAutomaticGateway(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/alipay/startAutomaticGateway',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("支付宝提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
            ...
            function startAutomaticLogOut(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/alipay/startAutomaticLogOut',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("支付宝提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
			...
			function del(id){
		              ...
		              function(){
		                 $.get("<?php echo url::s('admin/alipay/automaticDelete','id=');?>" + id, function(result){
		           ...
		           swal({
		              ...
		              function(){
				           $("input[name='items']:checked").each(function(){
				        	 $.get("<?php echo url::s('admin/alipay/automaticDelete','id=');?>" + $(this).val(), function(result){
