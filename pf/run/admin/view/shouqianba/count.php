    ...
    font-size: 14px;" onclick="exportDoc()">导出表格</button>
                            ...
                            <tr>
                                <form action="" method="get">
                                    ...
                                    <th>收钱吧ID：<input type="text" name="shouqianba_id" value="<?php if(!empty($_GET['shouqianba_id'])){  echo $_GET['shouqianba_id']; }?>"></th>
                            ...
                            <?php foreach ($result['result'] as $ru) { ?>
                                <tr>
                                    <td>收钱吧ID：<a
                                                href='<?php echo url::s("admin/shouqianba/automatic", "id={$ru['shouqianba_id']}"); ?>'><?php echo $ru['shouqianba_id']; ?></a>
                         ...
                         <?php echo $ru['shouqianba_id']; ?> | <?php echo $ru['id']; ?>
                                    ...
                                    <td>商户信息：<?php $userInfo = $mysql->query("client_user", "id={$ru['user_id']}")[0];
                                        echo is_array($userInfo) ? '<a href="' . url::s("admin/shouqianba/automaticOrder", "sorting=user&code={$userInfo[id]}&locking=true") . '"><span style="color:green;font-size:14px;font-weight:bold;">' . $userInfo['username'] . '</span></a>' . ' ( 商户ID: ' . $userInfo['id'] . ' ) ' : '<span style="color:red;font-size:8px;">会员不存在</span>'; ?>
                                        <br>手机号码：<span
            ...
            <script type="text/javascript">
                ...
                function exportDoc () {
                    var shouqianba_id = GetQueryString('shouqianba_id');
                    var user_id = GetQueryString('user_id');
                    var status = GetQueryString('status');
                    var start_time = GetQueryString('start_time');
                    var end_time = GetQueryString('end_time');
                    location.href = "<?php echo url::s('admin/shouqianba/export', "user_id=");?>" + user_id+ '&start_time='+start_time + '&end_time=' + end_time + '&status=' + status + 'shouqianba_id=' + shouqianba_id;
                }
