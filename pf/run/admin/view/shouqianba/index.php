            ...
            <?php  foreach ($result['result'] as $ru){?>
              <tr>
                <td>
                    ...
                    <p>备注: <?php echo $ru['name'] == '0' ? '<span style="color:red">Unused</span>' : '<span style="color:red">'.$ru['name'].'</span>';?>   ( <a href="<?php echo url::s('admin/nxys/automaticOrder',"sorting=shouqianba&code={$ru['id']}");?>">交易订单</a> )</p>
                ...
                <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_shouqianba_automatic_orders where shouqianba_id={$ru['id']} and creation_time > {$nowTime} and status=4");
                        $total = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_shouqianba_automatic_orders where shouqianba_id={$ru['id']} and creation_time > {$nowTime}");
                        if($total[0]['count']){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_shouqianba_automatic_orders where shouqianba_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4");
                        $total = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_shouqianba_automatic_orders where shouqianba_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime}");
                            if($total[0]['count']){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_shouqianba_automatic_orders where shouqianba_id={$ru['id']} and status=4");
                ...
                <p><a href="#" onclick="del('<?php echo $ru['id'];?>');" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i>移除收钱吧</a></p>
    ...
    <!-- End Panel -->  
            ...
            function startAutomaticRb(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/shouqianba/startAutomaticRb',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            ...
            function startAutomaticGateway(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/shouqianba/startAutomaticGateway',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("收钱吧提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
            ...
            function startAutomaticLogOut(id){
            	  swal({
                    ...
                    function(){
                       $.get("<?php echo url::s('admin/shouqianba/startAutomaticLogOut',"id=");?>" + id, function(result){
                      	 if(result.code == '200'){
            	            	swal("收钱吧提示", result.msg, "success");
            	              	setTimeout(function(){location.href = '';},1000);
            	              }else{
			...
			function del(id){
		              swal({
		                ...
		                showCancelButton: true, 
		                confirmButtonColor: "#DD6B55", 
		                confirmButtonText: "是的,我要删除该收钱吧!", 
		                closeOnConfirm: false 
		              },
		              function(){
		                 $.get("<?php echo url::s('admin/shouqianba/automaticDelete','id=');?>" + id, function(result){
			...
			function deletes(){ 
		           swal({
		                title: "非常危险", 
		                text: "你确定要批量删除已选中的收钱吧吗？", 
		                type: "warning", 
		                showCancelButton: true, 
		                confirmButtonColor: "#DD6B55", 
		                confirmButtonText: "是的,我要删除这些收钱吧!", 
		                closeOnConfirm: false 
		              },
		              function(){
				           $("input[name='items']:checked").each(function(){
				        	 $.get("<?php echo url::s('admin/shouqianba/automaticDelete','id=');?>" + $(this).val(), function(result){
