...
class index
{
    ...
    private function paofen($user, $type_content, $data)
    {
        ...
        if (!$create_order) {
            $this->mysql->rollBack();
            functions::str_json($type_content, -1, 'automatic->订单创建失败2,请联系客服');
        }
        ...
        if (!$st) {
            $this->mysql->rollBack();
            functions::str_json($type_content, -1, 'automatic->订单创建失败3,请联系客服');
        }
    ...
    private function alipay($user, $type_content, $data)
    {
        ...
        if ($create_order > 0) {
            ...
            if ($type_content == 'json') {
                ...
                //$json_qrcode = "https://ds.alipay.com/?from=mobilecodec&scheme=" . urlencode("alipays://platformapi/startapp?appId=10000007&qrcode=" . url::s("gateway/pay/alipayselect", "id={$create_order}"));
            ...
            //url::address(url::s("gateway/pay/alipayselect", "id={$create_order}"));
    ...
    private function service($user, $type_content, $data)
    {
        ...
        if ($data['type'] == 3) {
            $this->serviceBank($user, $type_content, $data, $where_gateway);
            return;
