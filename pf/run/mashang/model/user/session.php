...
class session{
    ...
    public function check($callType=''){
        ...
        if (!is_array($_SESSION['MEMBER'])) {
            ...
            if ($callType == 'json'){
                functions::json(403, '暂未登录,请登录后再访问',array('url'=>url::s('agent/user/login'),'time'=>date('Y/m/d H:i:s',time()),'ip'=>$Loca_ip));
            }else{
                url::address(url::s('mashang/user/login'),'请登录后再访问!',3);
            }
        }
        ...
        if (($_SESSION['MEMBER']['time'] + 1800) < time()) {
            ...
            if ($callType == 'json'){
                ...
                url::address(url::s('mashang/user/login'),'会话已经过期!',3);
            }
        }
           ...
           // unset($_SESSION['MEMBER']);
            if ($callType == 'json'){
               // functions::json(403, '账户异常,请重新登录',array('url'=>url::s('index/user/login'),'time'=>date('Y/m/d H:i:s',time()),'ip'=>$Loca_ip));
                ...
                //url::address(url::s('mashang/user/login'),'账户登录异常!',3);
            }
        }
        ...
        if (!is_array($_SESSION['MEMBER']['group']) || $_SESSION['MEMBER']['group']['authority'] == -1){
            unset($_SESSION['MEMBER']);
            if ($callType == 'json'){
                functions::json(403, 'BANNED',array('url'=>url::s('agent/user/login'),'time'=>date('Y/m/d H:i:s',time()),'ip'=>$Loca_ip));
            }else{
                url::address(url::s('mashang/user/login'),'BANNED!',3);
            }
        }
    ...
    static public function loginCheck($callType=''){
        ...
        if (isset($_SESSION['MEMBER']) && is_array($_SESSION['MEMBER'])) {
            ...
            if (($_SESSION['MEMBER']['time'] + 1800) > time()) {
                ...
                if (is_array($find_user) && md5($Loca_ip) === md5($_SESSION['MEMBER']['ip']) && md5($Loca_ip) === md5($find_user['ip'])) {
                    if ($callType == 'json'){
                        functions::json(200, '您已经登录过了,无需重复登录',array('url'=>url::s('agent/panel/home'),'time'=>date('Y/m/d H:i:s',time()),'ip'=>$Loca_ip));
                    }else{
                        url::address(url::s('mashang/panel/home'),'您已经登录过了!',3);
                    }
                }
            }
        }
