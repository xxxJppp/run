...
<title>代理登录 - <?php echo WEB_NAME; ?> - 支付技术服务商，让支付简单、专业、快捷！</title>
...
<div class="m-head">
	...
	<div class="m-head-nav">
		<ul class="c-wrapper">
			<li class="m-head-nav-logo" style="    font-size: 28px;
    font-weight: bold;
    line-height: 61px;
    margin-left: 70px;
    text-align: center;
...
<div class=" login2">
	<div class="login-wp">
		...
		<div class="login-bd">
			 <form id="profile" name="loginForm"  >
			<ul>
				<li>
...
<script>
  layui.use(['laydate', 'laypage', 'layer', 'form', 'element'], function() {
       ...
       form.on('submit(profile)', function(data){
            ...
            $.ajax({
            url:"<?php echo url::s('agent/user/dologin');?>",
            type:"post",
            data:$('#profile').serialize(),
            success:function(res){
                if(res.code == '200'){
                    layer.msg(res.msg, {icon: 1,time:1000,end:function () {
                            location.href="<?php echo url::s('agent/panel/home');?>";
                        }})
                }else{
