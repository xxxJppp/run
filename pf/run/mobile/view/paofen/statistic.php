...
<section id="content">
                                ...
                                <th>
                                    支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status') { ?>(<?php if ($sorting['code'] == 1) echo '获取订单中';
                                        ...
                                        if ($sorting['code'] == 4) echo '已支付'; ?>)<?php } ?><a
                                            href='<?php echo url::s('mashang/paofen/statisticOrder', "sorting=status&code=" . ($sorting['code'] + 1)); ?>'><i
                                ...
                                <th>
                                    异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback') { ?>(<?php if ($_GET['code'] == 0) echo '未回调';
                                        if ($_GET['code'] == 1) echo '已回调'; ?>)<?php } ?><a
                                            href='<?php echo url::s('mashang/paofen/statisticOrder', "sorting=callback&code=" . ($sorting['code'] + 1)); ?>'><i
                                                class="mdi-image-healing"></i></a></th>
    ...
    font-size: 14px;" onclick="exportData()">
                            ...
                            <?php echo $ru['paofen_id']; ?> | <?php echo $ru['id']; ?> </span>
                                    ...
                                    <td>盈利：<?php echo $ru['callback_status'] == 1 ? $ru['fees'] : '暂无信息'; ?>
                                        <br>
                                        接口返回信息：<span
...
<script type="text/javascript">
    ...
    function trade_no(obj) {
        ...
        if (start_time || end_time) {
            location.href = "<?php echo url::s('mashang/paofen/statisticOrder', "sorting=trade_no&");?>"+'&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('mashang/paofen/statisticOrder', "sorting=trade_no&code=");?>" + $(obj).val();
        }
    }

    function paofen() {
        var paofen = $('#paofen').val();
        location.href = "<?php echo url::s('mashang/paofen/statisticOrder', "sorting=paofen&code=");?>" + paofen;
    }
    ...
    function exportData () {
        ...
        if (code) {
            location.href = "<?php echo url::s('mashang/paofen/export', "sorting=paofen&code=");?>" + code+ '&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('mashang/paofen/export', "sorting=paofen&start_time=");?>" + start_time + '&end_time=' + end_time;
        }
    }
