...
class panel{
    ...
    public function __construct(){
      ...
      if($checkuser['is_mashang'] == 0){
         unset($_SESSION['MEMBER']);
        unset($_SESSION);
        url::address(url::s('mobile/user/login'), '您不是代理，请重新登录!', 0);
    ...
    public function index(){
       ...
       $where="uid =".$_SESSION['MEMBER']['uid'];
        $member = page::conduct('mashang_yajin_log', request::filter('get.page'), 10, $where, null, 'id', 'desc',"0,10");
...
//订单日志 押金
   ...
   public function paylog(){
       ...
       $where="uid =".$_SESSION['MEMBER']['uid'];
        $member = page::conduct('user_paylog', request::filter('get.page'), 10, $where, null, 'id', 'desc',"0,10");
        new view('panel/paylog', [
            'mysql'  => $this->mysql,
            'member' => $member
        ]);
  ...
  //订单列表
   public function order()
    {
        ...
        $where = "status = 4 and mashang_id ={$_SESSION['MEMBER']['uid']} and ";
   ...
   //会员列表
    public function mashanghuoli(){
        ...
        $where="uid =".$_SESSION['MEMBER']['uid'];
        $member = page::conduct('mashang_huoli_log', request::filter('get.page'), 10, $where, null, 'id', 'desc');
        $groups = $this->mysql->query("client_group");
        new view('panel/mashanghuoli', [
            'mysql'  => $this->mysql,
            'member' => $member,
            'groups' => $groups
        ]);
