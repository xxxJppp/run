...
class bank{
 ...
 public function update(){
  ...
  public function zz(){
		...
		$money="您账户1147于09月10日12:34银联入账人民币1.00元（支付宝（中国）网络技术有限公司/赵长春支付宝转账）[招商银行]";
        $money2="您账户1147于09月15日21:19收款人民币10.00，备注：财付通-财付通支付科技有限公司-，更多详情请查看招商银行APP动账通知。[招商银行]";
        $money3="您账户1147于09月10日12:26收款人民币200.00，备注：支付宝-支付宝（中国）网络技术有限公，更多详情请查看招商银行APP动账通知。[招商银行]";
        $money4="您账户1147于09月10日12:34银联入账人民币1.00元（支付宝（中国）网络技术有限公司/赵长春支付宝转账）[招商银行]";
  ...
  $money2 = str_replace(",","",$money2);
     ...
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
		...
		if($bank_phone == '95533' || $bank_phone == '9553395533'){//建设银行
        ...
        $find_order = $this->mysql->query('client_bank_automatic_orders',"app_user LIKE '{$mcid}' and status=2 and amount={$money2}")[0];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/bank/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $alipay_id
        ...
        //查询用户信息
        $find_uid = $this->mysql->query("client_bank_automatic_account","user_id={$user_id}")[0]['user_id'];
		
		file_put_contents("yinhang_uid.txt", json_encode( $find_uid));
        
        //检测是否已经有当前流水号了
        $cook = $this->mysql->query("client_pay_record","amount={$money2} and user_id={$find_uid} and types=2 and version_code='bank_auto' and bankorderid like '%{$tradeNo}%'")[0];
    ...
    public function callback(){
        ...
        $bank_id = request::filter('post.id');
		$order_id = request::filter('post.orderid');
        if (empty($bank_id)) functions::json(-1, 'ID错误');
        //服务信息
        $service = $this->mysql->query('client_bank_automatic_account',"id={$bank_id}")[0];
        if (!is_array($service)) functions::json(-1, '服务错误');
        // -------------------------
        // 获取需要回调的列表
       $order = $this->mysql->query('client_bank_automatic_orders', "id={$order_id} and status=4 and callback_status=0");
      ...
      //系统对代理的费率
       ...
       $shanghufees = $obj['amount'] * $shanghuauthority{$module_name};
        ...
        $this->mysql->update("client_bank_automatic_account", ['active_time'=>time()],"id={$bank_id}");
        functions::json(200, ' [' . date("Y/m/d H:i:s", time()) . ']: ID->' . $bank_id . ' 异步通知成功');
        //-----------------------------
    }
