...
class sqbalipay{
 ...
 public function update(){
    ...
    public function uploadOrder(){
     file_put_contents("shouqianba_log32.txt", json_encode($_POST));
	...
	$signn=md5($dt.$remark.$money.$no.$type."12345667");
      ...
      file_put_contents("shouqianba_sign.txt", $sign);
       file_put_contents("shouqianba_signn.txt", $signn);
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
		...
		// 把原先的订单号匹配改为收钱吧的流水号匹配
        $find_order32 = $this->mysql->query('client_shouqianba_automatic_orders',"sqbno='{$no}' and amount={$money} and app_user LIKE '{$appid}'")[0];
		if (is_array($find_order32)) {
          	$remark = $find_order32['trade_no'];
        }
        $find_order = $this->mysql->query('client_shouqianba_automatic_orders',"trade_no={$remark} and status=2 and amount={$money} and app_user LIKE '{$appid}'")[0];
        $order_id = $find_order['id'];
		$shouqianba_id = $find_order['shouqianba_id'];
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/sqbalipay/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $shouqianba_id
        ...
        //查询用户信息
        $find_uid = $this->mysql->query("client_shouqianba_automatic_account","user_id={$id}")[0]['user_id'];

        //检测是否已经有当前流水号了
        $cook = $this->mysql->query("client_pay_record","amount={$money} and user_id={$find_uid} and types=2 and version_code='shouqianba_auto' and trade_no like '%{$remark}%'")[0];
    ...
    public function callback(){
        ...
        $shouqianba_id = request::filter('post.id');
		$order_id = request::filter('post.orderid');
        if (empty($shouqianba_id)) functions::json(-1, 'ID错误');
        //服务信息
        $service = $this->mysql->query('client_shouqianba_automatic_account',"id={$shouqianba_id}")[0];
        if (!is_array($service)) functions::json(-1, '服务错误');
        // -------------------------
        // 获取需要回调的列表
       $order = $this->mysql->query('client_shouqianba_automatic_orders', "id={$order_id} and status=4 and callback_status=0");
      ...
      //系统对代理的费率
       ...
       $shanghufees = $obj['amount'] * $shanghuauthority{$module_name};
        ...
        $this->mysql->update("client_shouqianba_automatic_account", ['active_time'=>time()],"id={$shouqianba_id}");
        functions::json(200, ' [' . date("Y/m/d H:i:s", time()) . ']: ID->' . $shouqianba_id . ' 异步通知成功');
        //-----------------------------
    }
