...
class alipay{
  ...
  public function cs(){
    ...
    $find_order = $this->mysql->query("client_alipay_automatic_orders","status=2","trade_no");
  // $resWxIds = $this->mysql->query("client_paofen_automatic_account","status=4 and training=1 and receiving=1 and type={$type}","user_id");
   ...
   // foreach ($find_order as $obj) {
    var_dump($find_order['trade_no']);
 // }
 public function update(){
    ...
    public function uploadOrder(){
     ...
     file_put_contents("alipay_log32.txt", json_encode($_POST));
     ...
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('client_alipay_automatic_orders',"trade_no={$remark} and status=2 and amount={$money} and app_user LIKE '{$appid}'")[0];
       ...
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/alipay/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $alipay_id
        ...
        //检测是否已经有当前流水号了
        $cook = $this->mysql->query("client_pay_record","amount={$money} and user_id={$find_uid} and types=2 and version_code='alipay_auto' and trade_no like '%{$remark}%'")[0];
