...
class wechatdy{
 ...
 public function update(){
    ...
    public function uploadOrder(){
      ...
      file_put_contents("wechatdy_log.txt", json_encode($_POST));
   ...
   $b= (strpos($remark,"-"));
	...
	$signn=md5($dt.$remark.$money.$no.$type."12345667");
      ...
      file_put_contents("wechatdy_sign.txt", $sign);
       file_put_contents("wechatdy_signn.txt", $signn);
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('client_wechatdy_automatic_orders',"status=2 and amount={$money} and app_user LIKE '{$appid}' and dy_name LIKE '{$dy_name}'")[0];
        $order_id = $find_order['id'];
		$wechatdy_id = $find_order['wechatdy_id'];
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/wechatdy/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $wechatdy_id
        ...
        //查询用户信息
        $find_uid = $this->mysql->query("client_wechatdy_automatic_account","user_id={$id}")[0]['user_id'];

        //检测是否已经有当前流水号了
        $cook = $this->mysql->query("client_pay_record","amount={$money} and user_id={$find_uid} and types=2 and version_code='wechatdy_auto' and trade_no like '%{$remark}%'")[0];
    ...
    public function callback(){
        ...
        $wechatdy_id = request::filter('post.id');
		$order_id = request::filter('post.orderid');
        if (empty($wechatdy_id)) functions::json(-1, 'ID错误');
        //服务信息
        $service = $this->mysql->query('client_wechatdy_automatic_account',"id={$wechatdy_id}")[0];
        if (!is_array($service)) functions::json(-1, '服务错误');
        // -------------------------
        // 获取需要回调的列表
       $order = $this->mysql->query('client_wechatdy_automatic_orders', "id={$order_id} and status=4 and callback_status=0");
      ...
      //系统对代理的费率
       ...
       $shanghufees = $obj['amount'] * $shanghuauthority{$module_name};
        ...
        $this->mysql->update("client_wechatdy_automatic_account", ['active_time'=>time()],"id={$wechatdy_id}");
        functions::json(200, ' [' . date("Y/m/d H:i:s", time()) . ']: ID->' . $wechatdy_id . ' 异步通知成功');
        //-----------------------------
    }
