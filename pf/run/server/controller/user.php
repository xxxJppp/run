    ...
    function pay(){
     //   $this->keyVerification();
        ...
        if (is_array($find_user)){
           ...
           $in = $this->mysql->insert("user_paylog", [
            'uid'              => $user_id,
            'money'            => $amount,
            'old_money'        => $find_user['balance'],
            'new_money'        => $find_user['balance']+$amount,
            'remark'       => '原余额：'.$find_user['balance'].',充值金额：'.$amount .'，最终余额：'.$find_user['balance']+$amount,
            'time'            => time(),
