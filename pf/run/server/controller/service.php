...
class service
{
    ...
    {
        //file_put_contents("log.txt", date('Y-m-d H:i:s') . "===keyVerification===" . json_encode($_POST), FILE_APPEND);
    ...
    public function uploadOrder()
    {
        ...
        $today_money = floatval($today_money);

        $today_pens = intval(request::filter('post.today_pens'));
        $find_order = $this->mysql->query('service_order', "service_id={$id} and status=2 and amount={$money} and id={$order}")[0];
        ...
        $result = callbacks::sendRequest(URL_ROOT . '/server/service/callback', http_build_query(['id' => $id]));
        functions::json(200, ' [' . date("Y/m/d H:i:s", time()) . ']: 服务ID->' . $order . ' 订单处理完成' . json_encode($result));
    }
    ...
    public function uploadOrderBank()
    {
        ...
        $amount_true = floatval($amount_true);
        $newTime = time();
        //$no = request::filter('post.no');
        //$order = trim(request::filter('post.order'));
        ...
        $find_order = $this->mysql->query('service_order', "status=2 and amount_true={$amount_true} and bank_acount='{$bank_acount}' and expire_time>{$newTime}");
        //找到订单数量超过2条，则记录未匹配订单
        if(count($find_order)>1){
            //插入无匹配订单，删除订单
            foreach($find_order as $k=>$v) {
                $status = $this->mysql->insert('service_order_no', [
        ...
        if (is_array($find_order)&&!empty($find_order)) {
            $status = $this->mysql->update("service_order", [
                'status'   => 4,
                'pay_time' => time(),
            ], "id={$find_order[0]['id']}");
        }else{
        ...
        $this->mysql->insert("client_pay_record", [
            'pay_time'     => time(),
            'amount'       => $amount_true,
            'user_id'      => $find_order[0]['user_id'],
            'pay_note'     => '服务版' . ' - 订单信息：' . $find_order[0]['trade_no'],
            'types'        => $find_order[0]['types'],
            'version_code' => 'service_auto',
            'average'      => 1
        ]);
        if($status){
            ...
            echo 'error:找不到订单' . "status=2 and amount_true={$amount_true} and bank_acount={$bank_acount}";
        }

        $result = callbacks::sendRequest(URL_ROOT . '/server/service/callback', http_build_query(['id' => $find_order[0]['service_id']]));
    ...
    public function callback()
    {
        ...
        // -------------------------
        // 获取需要回调的列表
        $order = $this->mysql->query('service_order', "service_id={$service_id} and status=4 and callback_status=0 and callback_count<4 limit 0,30");
...
//                    }
                    ...
                    $callback_time = time();
                    // 手续费扣除成功，开始回调
                    $http_build_data = callbacks::sendRequest($obj['callback_url'], http_build_query([
                    ...
                    $callback_status =  (strtolower($http_build_data['msg']) == 'success') ? 1:0;
                    //更新订单
                    $this->mysql->update("service_order", [
                        'callback_time'    => $callback_time,
                        'callback_status'  => $callback_status,
                        'callback_content' => htmlentities($http_build_data['msg']),
                        'fees'             => $fees,
                        'agent_rate'       => $agent_rate,
                        'reached'          => 0,
                        'callback_count'   => $obj['callback_count']+1
                    ], "id={$obj['id']} AND  reached = 0");
...
//                // 手续费扣除成功，开始回调
    ...
    public function pcUploadOrder()
    {
        ...
        $find_order = $this->mysql->query('service_order', "service_id={$id} and status=2 and amount={$money} and id={$order}")[0];
        if (is_array($find_order)) {
            $update_id = $this->mysql->update("service_order", [
                'status'        => 4,
                'pay_time'      => time(),
                'callback_from' => 'pc',
                'no'            => $no,
            ...
            $order = $this->mysql->query('service_order', "id={$order}", 'status,amount')[0];
