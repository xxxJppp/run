...
class lakala{
 ...
 public function update(){
    ...
    public function uploadOrder(){
     ...
     file_put_contents("lakala_log.txt", json_encode($_POST));
	...
	$signn=md5($dt.$remark.$money.$no.$type."12345667");
      ...
      file_put_contents("lakala_sign.txt", $sign);
       file_put_contents("lakala_signn.txt", $signn);
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('client_lakala_automatic_orders',"status=2 and amount={$money} and app_user LIKE '{$appid}'")[0];
       ...
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/lakala/callback.do', http_build_query([ 
                                'orderid' => $order_id,
                                'id' => $lakala_id
