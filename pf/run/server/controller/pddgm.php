...
class pddgm{
 ...
 public function update(){
    ...
    public function uploadOrder(){
      ...
      file_put_contents("pddgm_log.txt", json_encode($_POST));
	...
	$signn=md5($dt.$remark.$money.$no.$type."12345667");
      ...
      file_put_contents("pddgm_sign.txt", $sign);
       file_put_contents("pddgm_signn.txt", $signn);
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       ...
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/pddgm/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $pddgm_id
