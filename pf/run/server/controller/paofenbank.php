...
class paofenali{
     ...
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('client_paofen_automatic_orders',"status=2 and type=5 and amount={$money2} and app_user LIKE '{$mcid}'")[0];
    ...
    public function callback(){
        ...
        // -------------------------
        // 获取需要回调的列表
       $order = $this->mysql->query('client_paofen_automatic_orders', "id={$order_id} and status=4 and callback_status=0");
    ...
    $pankouhuoli = $obj['amount'] - $pankoufees;
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
              ...
              //写盘口获利记录
                     $this->mysql->insert("pankou_huoli_log", [
                          'uid' => $puser['id'],
                          'orderid' => $order_id,
                          'amount'    => $obj['amount'],
               ...
               $updateStatus_pankou =$this->mysql->update("client_user", ['balance' => $pankou_balance], "id={$puser['id']}");
                ...
                //写押金记录 冻结金额
                     $this->mysql->insert("mashang_yajin_log", [
                         ...
                         'remark'    =>'APP回调成功！订单号：'.$obj['trade_no'].',扣除金额：'.$obj['amount'].'元，扣除前余额：'. $user['balance'].'元，扣除后余额：'. $mauser['balance'].'元',
                         'time' => time(),
                        'status' => 2
