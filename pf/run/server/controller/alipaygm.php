...
class alipaygm{
 ...
 public function update(){
    ...
    public function uploadOrder(){
     ...
     file_put_contents("alipaygmmmmmmmmmmmmmm_log32.txt", json_encode($_POST));
	...
	$signn=md5($dt.$remark.$money.$no.$type."12345667");
      ...
      file_put_contents("alipaygm_sign.txt", $sign);
       file_put_contents("alipaygm_signn.txt", $signn);
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('client_alipaygm_automatic_orders',"status=2 and amount={$money} and app_user LIKE '{$appid}'")[0];
        $order_id = $find_order['id'];
		$alipaygm_id = $find_order['alipaygm_id'];
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/alipaygm/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $alipaygm_id
        ...
        //查询用户信息
        $find_uid = $this->mysql->query("client_alipaygm_automatic_account","user_id={$id}")[0]['user_id'];

        //检测是否已经有当前流水号了
        $cook = $this->mysql->query("client_pay_record","amount={$money} and user_id={$find_uid} and types=2 and version_code='alipaygm_auto' and trade_no like '%{$remark}%'")[0];
