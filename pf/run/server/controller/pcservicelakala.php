    ...
    if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('service_order',"status=2 and amount={$price1} and types=4 and app_user LIKE '{$appid}'")[0];
       ...
       $id = $find_order['id'];
        ...
        if (is_array($find_order)) {
            ...
            $result = callbacks::curl('http://'.DOMAINS_URL.'/server/servicelakala/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $service_id
        ...
        $this->mysql->insert("client_pay_record", [
            ...
            'amount'=>$money,
            'user_id'=>$find_uid,
            'pay_note'=>'[拉卡拉转账]ID：'.$id . $remark,
            'types'=>2,
            'version_code'=>'alipay_auto',
            'average'=>$average
        ]);
        functions::json(200, ' ['.date("Y/m/d H:i:s",time()).']: 拉卡拉转账ID->' . $id . ' 订单处理完成');
