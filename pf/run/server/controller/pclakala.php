...
class pclakala{
 ...
 public function update(){
			 ...
			 $this->mysql->update("client_lakala_automatic_orders", [
                'status'=> 3
    ...
    public function uploadOrder(){
      ...
      file_put_contents("lakalapc_log.txt", json_encode($_POST));
    //  {"cardnumlast":"Mardan(**\u73ed)","trantime":"2019-08-09 23:40:27","device":"a111","channel":"\u5fae\u4fe1\u6536\u6b3e","amount":"0.02","cardcode":"lakala","sign":"43747c46e3cfef2e6f5736caba6cae3b"}
    ...
    $price = request::filter('post.price');
   $price = request::filter('post.price');
    $p= substr($price, 0,4);
	
		if($p < 1){
			$price1= substr($price, 0,4);	
		}else if($p < 10){
			$price1= substr($price, 0,5);	
		}else if($p < 100){
			$price1= substr($price, 0,6);	
		}else if($p < 1000){
			$price1= substr($price, 0,7);	
		}else if($p < 10000){
			$price1= substr($price, 0,8);	
		}
   ...
   //  取数据摘要 (到字节集 (键值表.取文本 (“银行简称”) ＋ 金额 ＋ “huoshan111” ＋ 键值表.取文本 (“银行尾号”) ＋ 键值表.取文本 (“time”)))
	...
	$signn=md5($type.$money."huoshan111".$trantime);
      ...
      file_put_contents("lakaladypc_sign.txt", $sign);
       file_put_contents("lakaladypc_signn.txt", $signn);
    if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('client_lakala_automatic_orders',"status=2 and amount={$price1} and app_user LIKE '{$appid}'")[0];
        $order_id = $find_order['id'];
		$lakala_id = $find_order['lakala_id'];
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/pclakala/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $lakala_id
        ...
        //查询用户信息
        $find_uid = $this->mysql->query("client_lakala_automatic_account","user_id={$id}")[0]['user_id'];

        //检测是否已经有当前流水号了
        $cook = $this->mysql->query("client_pay_record","amount={$money} and user_id={$find_uid} and types=2 and version_code='lakala_auto' and trade_no like '%{$remark}%'")[0];
    ...
    public function callback(){
        ...
        $lakala_id = request::filter('post.id');
		$order_id = request::filter('post.orderid');
        if (empty($lakala_id)) functions::json(-1, 'ID错误');
        //服务信息
        $service = $this->mysql->query('client_lakala_automatic_account',"id={$lakala_id}")[0];
        if (!is_array($service)) functions::json(-1, '服务错误');
        // -------------------------
        // 获取需要回调的列表
       $order = $this->mysql->query('client_lakala_automatic_orders', "id={$order_id} and status=4 and callback_status=0");
      ...
      //系统对代理的费率
       ...
       $shanghufees = $obj['amount'] * $shanghuauthority{$module_name};
        ...
        $this->mysql->update("client_lakala_automatic_account", ['active_time'=>time()],"id={$lakala_id}");
        functions::json(200, ' [' . date("Y/m/d H:i:s", time()) . ']: ID->' . $lakala_id . ' 异步通知成功');
        //-----------------------------
    }
