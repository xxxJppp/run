     ...
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('client_paofen_automatic_orders',"status=2 and type=2 and amount={$money} and app_user LIKE '{$appid}'")[0];
       ...
       $id = $find_order['user_id'];
        ...
        if (is_array($find_order)) {
			...
			$result = callbacks::curl('http://'.DOMAINS_URL.'/server/paofenwechat/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $paofen_id
