     ...
     if($signn !== $sign){
      functions::json(200, 'sign不匹配');
       exit;
        ...
        $find_order = $this->mysql->query('service_order',"app_user LIKE '{$mcid}' and status=2 and types=3 and amount={$money2}")[0];
       ...
       $id = $find_order['id'];
        ...
        if (is_array($find_order)) {
            ...
            $result = callbacks::curl('http://'.DOMAINS_URL.'/server/servicebank/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $service_id
