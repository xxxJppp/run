   ...
   public function update(){
    ...
    if($signn !== $sign){
     ...
     exit;
        ...
        $find_order = $this->mysql->query('service_order',"status=2 and amount={$money} and types=1 and app_user LIKE '{$appid}'")[0];
        ...
        if (is_array($find_order)) {
            ...
            $result = callbacks::curl('http://'.DOMAINS_URL.'/server/servicewechat/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $service_id
