   ...
   public function update(){
    ...
    if($signn !== $sign){
     ...
     exit;
        ...
        $find_order = $this->mysql->query('service_order',"status=2 and amount={$money} and types=13 and app_user LIKE '{$appid}'")[0];
        ...
        if (is_array($find_order)) {
            ...
            $result = callbacks::curl('http://'.DOMAINS_URL.'/server/servicealipaygm/callback.do', http_build_query([
                                'orderid' => $order_id,
                                'id' => $service_id
        ...
        $this->mysql->insert("client_pay_record", [
            ...
            'amount'=>$money,
            'user_id'=>$find_uid,
            'pay_note'=>'[支付宝固码转账]ID：'.$id . $remark,
            'types'=>2,
            'version_code'=>'wechat_auto',
            'average'=>$average
        ]);
        functions::json(200, ' ['.date("Y/m/d H:i:s",time()).']: 支付宝固码转账ID->' . $id . ' 订单处理完成');
