...
class wechatAutomatic
{
    ...
    public function uploadOrder()
    {
        ...
        $find_order = $this->mysql->query('client_wechat_automatic_orders', "wechat_id={$id} and status=2 and amount={$money} and id={$order}")[0];
        ...
        $url = URL_ROOT . '/server/callback/automatic';
        $data = [];
        $data['id'] = $id;
        functions::curlPostRequest($url, $data);
//        $result = callbacks::curl(URL_ROOT . '/server/callback/automatic', http_build_query(['id' => $id]));
        functions::json(200, ' [' . date("Y/m/d H:i:s", time()) . ']: 这里是微信回调-微信ID->' . $order . ' 订单处理完成');
    }
