...
-->
          ...
          <div class="container">
            <div class="row">
              <div class="col s12 m12 l12">
                <h5 class="breadcrumbs-title">微信店员固码支付</h5>
                <ol class="breadcrumbs">
                    <li><a href="<?php echo url::s('index/panel/home');?>">仪表盘</a></li>
                    <li><a href="#">微信店员信息列表</a></li>
        ...
        <div class="container">
          <div class="section">
            ...
            <p class="caption">
            <a href="<?php echo url::s("index/wechatdy/automaticOrder");?>" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left" style="width: 10px;"></i>全部订单</a>
            <a onclick="add();" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-content-add left" style="width: 10px;"></i>添加微信店员</a>
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        ...
                        <td>
                         ...
                         微信名称: <?php echo $ru['dy_name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['dy_name'].'</span>';?>[ <a href="#" onclick="editdyname('<?php echo $ru['id'];?>');">修改名称</a> ] </br>
						 备注: <?php echo $ru['name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['name'].'</span>';?><br/>微信店员二维码: <?php echo $ru['ewmurl'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['ewmurl'].'</span>';?>
                              <br>( <a href="#" onclick="del('<?php echo $ru['id'];?>');" style="color:#757575;">删除微信店员账户</a> )
                        </td>
                           <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechatdy_automatic_orders where wechatdy_id={$ru['id']} and creation_time > {$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $today_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechatdy_automatic_orders where wechatdy_id={$ru['id']} and creation_time > {$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechatdy_automatic_orders where wechatdy_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $yesterday_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechatdy_automatic_orders where wechatdy_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechatdy_automatic_orders where wechatdy_id={$ru['id']} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
      ...
      <script type="text/javascript">
          ...
          swal({   title: "请选择收款地区",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/wechatdy/areaAdd',"id=");?>" + id + "&area=" +$('#area').val() , function(result){
                      if(result.code == '200'){
      ...
      function add(){
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['620px', '560px'],
			  content: '<?php echo url::s('index/wechatdy/viewAdd');?>' //iframe的url
			}); 
	  }
         ...
         function edit(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/wechatdy/automaticEditName',"id=");?>" + id + "&app_user=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("微信店员提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
                          swal.showInputError(result.msg);
                      }
      ...
      function startAutomaticRb(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/wechatdy/startAutomaticRb',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("微信店员提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
    	            	swal("微信店员提示", result.msg, "error");
    	              }
              	  });
            });	
      }
         ...
         function editdyname(id){
          swal({   title: "修改提醒",
                  ...
                  closeOnConfirm: false,
                  animation: "slide-from-top",
                  inputPlaceholder: "店长微信名称+（张**）",
                  confirmButtonText: "确认提交" },
              function(inputValue){
                  if (inputValue === false) return false;
                  if (inputValue === "") {
                      swal.showInputError("店长微信名称+（张**）");
                      return false
                  }
                  $.get("<?php echo url::s('index/wechatdy/automaticEditdyName',"id=");?>" + id + "&dy_name=" + inputValue, function(result){
                      if(result.code == '200'){
      ...
      function startAutomaticGateway(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/wechatdy/startAutomaticGateway',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("微信店员提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticLogOut(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/wechatdy/startAutomaticLogOut',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("微信店员提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function login(id){
    	  swal({   title: "微信店员登录",   
              ...
              function(){
              //开始请求微信店员登录
            	   $.get("<?php echo url::s('index/wechatdy/startAutomaticLogin',"id=");?>" + id, function(result){
                  	 if(result.code == '200'){
                   		    $('.showSweetAlert p').html(result.msg);
                     		login_listen(id);
        	              }else{
        	            	swal("微信店员登录", result.msg, "error");
        	             }
              		});
      ...
      function login_listen(id){
      	listen_login = setInterval(function(){ $.get("<?php echo url::s('index/wechatdy/getAutomaticStatus',"id=");?>" + id, function(result){
      		if(result.code > 0){	
      			if(result.code == '2' || result.code == '3' ){ $('.showSweetAlert p').html(result.msg); }
      			if(result.code == '7'){
				//将二维码展现出来扫码
      				$('.showSweetAlert h2').html('请使用微信店员扫一扫');
      				$('.showSweetAlert p').html("<img style='width:200px;height:200px;' src='data:image/png;base64," + result.data.img + "'/>");
      				if(music == 0){
      					play(['<?php echo FILE_CACHE . "/download/sound/微信店员扫一扫1.mp3";?>']);
      					music = 1;
          			}
              	}
      			if(result.code == '4'){
	            	swal("微信店员登录", result.msg, "success");
	              	setTimeout(function(){location.href = '';},1000);
                }
             }else{
            	 swal("微信店员登录", result.msg, "error");
	  ...
	  function del(id){
		  swal({   title: "微信店员提醒",   
             ...
             $.get("<?php echo url::s('index/wechatdy/automaticDelete',"id=");?>" + id + "&pwd=" + inputValue, function(result){
              	 if(result.code == '200'){
               		    swal("微信店员提醒", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
	 ...
	 //轮训测试
	  function robinTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/wechatdy/robinTest");?>' + '?amount=' + $('#amount').val() + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function gatewayTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/wechatdy/gatewayTest");?>' + '?amount=' + $('#amount').val()  + "&keyId=" + id + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
         ...
         function editMaxAmount(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/wechatdy/editMaxAmount',"id=");?>" + id + "&amount=" + inputValue, function(result){
                      if(result.code == '200'){
         ...
         function editMaxdd(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/wechatdy/editMaxdd',"id=");?>" + id + "&dd=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
	  ...
	  function setting(){ 
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['600px', '400px'],
			  content: '<?php echo url::s('index/wechatdy/automaticConfig');?>' //iframe的url
			}); 
	  }
