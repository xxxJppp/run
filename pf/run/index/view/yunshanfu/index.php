    ...
    <!-- START CONTENT -->
      <section id="content">
          ...
          <div class="container">
            <div class="row">
              <div class="col s12 m12 l12">
                <h5 class="breadcrumbs-title">云闪付 <?php echo SYSTEM_VERSION; ?></h5>
                <ol class="breadcrumbs">
                    <li><a href="#">云闪付收款账户</a></li>
        ...
        <div class="container">
          <div class="section">
            ...
            <p class="caption">
            <a href="<?php echo url::s("index/yunshanfu/automaticOrder");?>" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left" style="width: 10px;"></i>全部订单</a>
            
            <a onclick="addNew();" style="font-size: 14px;background-color: red;!important;" class="btn waves-effect waves-light  cyan " ><i class="mdi-content-add left" style="width: 10px;"></i>添加收款账号</a>
                  ...
                  <table class="striped"  style="font-size: 14px;">
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        ...
                        账户: <?php echo $ru['name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['name'].'</span>';?><br/>云闪付账号: <?php echo $ru['account_no'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['account_no'].'</span>';?>
                              <br>( <a href="#" onclick="del('<?php echo $ru['id'];?>');" style="color:#757575;">删除云闪付卡账户</a> )
                        </td>
                           <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_yunshanfu_automatic_orders where yunshanfu_id={$ru['id']} and creation_time > {$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $today_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_yunshanfu_automatic_orders where yunshanfu_id={$ru['id']} and creation_time > {$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_yunshanfu_automatic_orders where yunshanfu_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $yesterday_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_yunshanfu_automatic_orders where yunshanfu_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_yunshanfu_automatic_orders where yunshanfu_id={$ru['id']} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
      ...
      <script type="text/javascript">
          function add(){
              swal({
                  ...
                  function(){
                      $.get("<?php echo url::s('index/yunshanfu/automaticAdd');?>", function(result){
                          if(result.code == '200'){
      ...
      function addNew(){
          swal({   title: "请输入账户配置信息",
                  ...
                  closeOnConfirm: false,
                  html: true,
                  text: "账户（云闪付号）<input type='text' id='account_no' value=''>"
...
,
              ...
              function(inputValue){
                  ...
                  if(account_no == false || account_no == null){
                      swal.showInputError("云闪付账号不能为空");
                      return false
                  }
                  $.get("<?php echo url::s('index/yunshanfu/automaticAdd',"name=");?>" +$('#name').val() +'&account_no='+account_no, function(result){
                      if(result.code == '200'){
      ...
      function withdraw(key_id,status) {
          ...
          swal({
              ...
              function (inputValue) {
                  ...
                  $.get("<?php echo url::s('index/yunshanfu/withdraw', "id=");?>" + id + "&pwd=" + inputValue +"&key_id="+key_id +"&type=alipay", function (result) {
                      if (result.code == '200') {
      ...
      function editMaxAmount(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  if (inputValue === false) return false;
                  if (inputValue === "") {
                      swal.showInputError("请输入当天最大收款金额!");
                      return false
                  }
                  $.get("<?php echo url::s('index/yunshanfu/editMaxAmount',"id=");?>" + id + "&amount=" + inputValue, function(result){
                      if(result.code == '200'){
          ...
          swal({   title: "请选择收款地区",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/yunshanfu/areaAdd',"id=");?>" + id + "&area=" +$('#area').val() , function(result){
                      if(result.code == '200'){
                          swal("成功提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
         ...
         function editMaxdd(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/yunshanfu/editMaxdd',"id=");?>" + id + "&dd=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function editNote(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/yunshanfu/editNote',"id=");?>" + id + "&note=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function edit(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/yunshanfu/automaticEditName',"id=");?>" + id + "&app_user=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("云闪付提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
                          swal.showInputError(result.msg);
                      }
      ...
      function startAutomaticRb(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/yunshanfu/startAutomaticRb',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("账户提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticGateway(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/yunshanfu/startAutomaticGateway',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("账户提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticLogOut(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/yunshanfu/startAutomaticLogOut',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("账户提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function login(id){
    	  swal({   title: "账户登录",   
              ...
              function(){
              //开始请求账户登录
            	   $.get("<?php echo url::s('index/yunshanfu/startAutomaticLogin',"id=");?>" + id, function(result){
                  	 if(result.code == '200'){
      ...
      function login_listen(id){
      	listen_login = setInterval(function(){ $.get("<?php echo url::s('index/yunshanfu/getAutomaticStatus',"id=");?>" + id, function(result){
      		if(result.code > 0){	
	  ...
	  function del(id){
		  swal({   title: "账户提醒",   
             ...
             $.get("<?php echo url::s('index/yunshanfu/automaticDelete',"id=");?>" + id + "&pwd=" + inputValue, function(result){
              	 if(result.code == '200'){
	 ...
	 //轮训测试
	  function robinTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/yunshanfu/robinTest");?>' + '?amount=' + $('#amount').val() + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function gatewayTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/yunshanfu/gatewayTest");?>' + '?amount=' + $('#amount').val()  + "&keyId=" + id + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function setting(){ 
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['600px', '400px'],
			  content: '<?php echo url::s('index/yunshanfu/automaticConfig');?>' //iframe的url
			}); 
	  }
