...
<section id="content">
    ...
    <div class="container">
        <div class="section">
            ...
            <p class="caption">
                <a href="<?php echo url::s("index/yunshanfu/automatic"); ?>" style="font-size: 14px;"
                   class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left"
                                                                          style="width: 10px;"></i>云闪付列表</a>
                ...
                <span style="font-size: 15px;margin-left:20px;">[ <b>今日收入:</b> <?php //查询今日收入
                    ...
                    ?>
                        ...
                        <a href="<?php echo url::s("admin/yunshanfu/statisticOrder", "sorting=user&code={$_GET['code']}&locking=true"); ?>"
                           style="color: green;">锁定该用户查询</a> ]<?php } ?> <?php if ($_SESSION['ALIPAY']['WHERE'] != '') { ?>  [
                        <a href="<?php echo url::s("admin/yunshanfu/statisticOrder", "sorting=user&code={$_GET['code']}&locking=false"); ?>"
                                ...
                                <th>
                                    <div class="input-field col s6" style="font-weight:normal;width:100%;">
                                        <select multiple id="alipay">
                                            <option value="" disabled selected>选择云闪付</option>
                                        ...
                                        <label>选择通道来查看<?php if ($_SESSION['ALIPAY']['ORDER']['WHERE'] == '') { ?>(<a
                                                    ...
                                                    href="<?php echo url::s('index/yunshanfu/statisticOrder', "sorting=alipay&locking=closed"); ?>">取消锁定</a>)<?php } ?>
                                        </label>
                                    </div>
                                </th>
                                <th>
                                    支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status') { ?>(<?php if ($sorting['code'] == 1) echo '获取订单中';
                                        ...
                                        if ($sorting['code'] == 4) echo '已支付'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index/yunshanfu/statisticOrder', "sorting=status&code=" . ($sorting['code'] + 1)); ?>'><i
                                ...
                                <th>
                                    异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback') { ?>(<?php if ($_GET['code'] == 0) echo '未回调';
                                        if ($_GET['code'] == 1) echo '已回调'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index/yunshanfu/statisticOrder', "sorting=callback&code=" . ($sorting['code'] + 1)); ?>'><i
                                                class="mdi-image-healing"></i></a></th>
                                <th>回调发送次数</th>
                                <th>回调来源</th>
                                <th>回调信息</th>
    ...
    font-size: 14px;" onclick="exportData()">
                            ...
                            <?php foreach ($result['result'] as $ru) { ?>
                                <tr>
                                    <td>云闪付账号ID：<a
                                                href='<?php echo url::s("index/yunshanfu/automatic"); ?>'><?php echo $ru['yunshanfu_id']; ?></a>
                                        / 订单ID：<?php echo $ru['id']; ?> ( <a target="_blank"
                                                                             href="<?php echo url::s('gateway/pay/automaticyunshanfu', "id={$ru['id']}"); ?>">支付链接</a>
                                        )
                            ...
                            <?php echo $ru['alipay_id']; ?> | <?php echo $ru['id']; ?> </span>
...
<script type="text/javascript">
    function reissue(id) {
        swal({
            ...
            function () {
                //开始请求云闪付登录
                $.get("<?php echo url::s('index/yunshanfu/automaticReissue', "id=");?>" + id, function (result) {
                    if (result.code == '200') {
                        swal("云闪付提示", result.msg, "success");
                        setTimeout(function () {
                            location.href = '';
                        }, 1000);
                    } else {
    ...
    function trade_no(obj) {
        ...
        if (start_time || end_time) {
            location.href = "<?php echo url::s('index/yunshanfu/statisticOrder', "sorting=trade_no&");?>"+'&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('index/yunshanfu/statisticOrder', "sorting=trade_no&code=");?>" + $(obj).val();
        }
    }

    function alipay() {
        var alipay = $('#alipay').val();
        location.href = "<?php echo url::s('index/yunshanfu/statisticOrder', "sorting=alipay&code=");?>" + alipay;
    }
    ...
    function exportData () {
        ...
        if (code) {
            location.href = "<?php echo url::s('index/yunshanfu/export', "sorting=alipay&code=");?>" + code+ '&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('index/yunshanfu/export', "sorting=alipay&start_time=");?>" + start_time + '&end_time=' + end_time;
        }
    }
