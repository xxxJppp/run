    ...
    <!-- START CONTENT -->
      <section id="content">
        ...
        <div class="container">
          <div class="section">
            ...
            <a onclick="addNew();" style="font-size: 14px;background-color: red;!important;" class="btn waves-effect waves-light  cyan " ><i class="mdi-content-add left" style="width: 10px;"></i>添加支付宝固码</a>
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        ...
                        APP商户号: <?php echo $ru['app_user'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['app_user'].'</span>';?>[ <a href="#" onclick="edit('<?php echo $ru['id'];?>');">修改名称</a> ] <font color=red>( <?php if($ru['is_hongbao'] == '1'){ echo "红包模式"; }else { echo "转账模式" ;}  ?> )</font> <br>
                           ...
                           收款码链接: <?php echo $ru['pid'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['pid'].'</span>';?>
      ...
      function addNew(){
          swal({   title: "请输入支付宝配置信息",
                  ...
                  closeOnConfirm: false,
                  html: true,
                  text: "支付宝收款码链接（<a href='https://jiema.wwei.cn/' target='_blank'>点击这里解析二维码</a>）<input type='text' id='pid' value=''>"
                  +"备注<input type='text' id='name' value=''>",
                  confirmButtonText: "提交" },
              function(inputValue){
                  if (inputValue === false) return false;
                  if (inputValue === "") {
                      swal.showInputError("支付宝收款二维码不能为空");
                      return false
                  }
                  $.get("<?php echo url::s('index/alipaygm/automaticAdd',"name=");?>" +$('#name').val() +'&pid='+ $('#pid').val()+'&is_new_version=1', function(result){
                      if(result.code == '200'){
