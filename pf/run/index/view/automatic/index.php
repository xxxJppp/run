...
<section id="content">
    ...
    <div class="container">
        <div class="section">
            ...
            <p class="caption">
                <a href="<?php echo url::s("index/wechat/automaticOrder"); ?>" style="font-size: 14px;"
                   class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left"
                                                                          style="width: 10px;"></i>全部订单</a>
                <a onclick="add();" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i
                            class="mdi-content-add left" style="width: 10px;"></i>添加微信</a>
                <a onclick="setting();" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i
                            class="mdi-action-settings left" style="width: 10px;"></i>微信配置</a>
                <a onclick="robinTest();" style="font-size: 14px;"
                   class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-hardware-gamepad left"
                                                                          style="width: 10px;"></i>通道测试（轮训）</a>
                <a onclick="apk();" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i
                            class="mdi-file-cloud-download left" style="width: 10px;"></i>助手 APP下载</a>
                <a onclick="pc();" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i
                            class="mdi-hardware-desktop-windows left" style="width: 10px;"></i>PC监控下载</a>
                <a href="<?php echo URL_ROOT . '/download/Android.zip' ?>" style="font-size: 14px;"
                   class="btn waves-effect waves-light cyan darken-2"><i class="mdi-hardware-memory left"
                                                                         style="width: 10px;"></i>下载安卓环境</a>
            </p>
                        ...
                        <table class="striped" style="font-size: 14px;">
                            ...
                            <?php foreach ($result['result'] as $ru) { ?>
                                <tr>
                                    ...
                                    <td>
                                        Name: <?php echo $ru['name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">' . $ru['name'] . '</span>'; ?>
                                        [ <a href="#" onclick="edit('<?php echo $ru['id']; ?>');">修改名称</a> ]<br>[ 今日总额:
                                        <span style="color: green;"><?php echo $ru['today_money']; ?></span> / 笔数: <span
                                                style="color: red;"><?php echo $ru['today_pens']; ?></span> ]<br> ( <a
                                                href="#" onclick="del('<?php echo $ru['id']; ?>');"
                                                style="color:#757575;">删除微信</a> )
                                        <br>Status: <?php echo (new model())->load('wechat', 'distinguish')->status($ru['status']);
                                        if ($ru['status'] != 6 && $ru['status'] != 1) echo ' ( <a href="#" style="color:red;" onclick="startAutomaticLogOut(' . $ru['id'] . ');">安全下线</a> )'; ?>
                                        <br>Last
                                        login: <?php echo $ru['login_time'] == 0 ? '从未登录' : date("Y/m/d H:i:s", $ru['login_time']); ?>
                                        <br>
                                    </td>
                                    <td>
                                        <?php echo $ru['max_amount'] ?><br>
                                        [ <a href="#" onclick="editMaxAmount('<?php echo $ru['id']; ?>');">修改</a> ]
                                        <br/> <br/> <br/>
                                        [ <a href="#" onclick="withdraw('<?php echo $ru['key_id']; ?>','<?php echo $ru['status']; ?>');">我要提现</a> ]
                                    ...
                                    <td>
                                        <b>ACTIVE
                                            Time:</b> <?php echo $ru['active_time'] == 0 ? '从未登录' : date("Y/m/d H:i:s", $ru['active_time']); ?>
                                        <br>
                                        <b>ACTIVE Number:</b> <?php echo $ru['heartbeats']; ?><br>
                                        <b>Android
                                            Time:</b> <?php echo $ru['android_heartbeat'] == 0 ? '无信息' : date("Y/m/d H:i:s", $ru['android_heartbeat']); ?>
                                    </td>
                                    ...
                                    <td>
                                        <?php echo $ru['note'] ?>
                                        <br>
                                        [ <a href="#" onclick="editNote('<?php echo $ru['id']; ?>');">修改</a> ]
                                    </td>
                                </tr>
                            <?php } ?>
                            </tbody>
...
<script type="text/javascript">
    ...
    function login_listen(id) {
        listen_login = setInterval(function () {
            $.get("<?php echo url::s('index/wechat/getAutomaticStatus', "id=");?>" + id, function (result) {
                if (result.code > 0) {
    ...
    function edit(id) {
        swal({
                ...
                closeOnConfirm: false,
                animation: "slide-from-top",
                inputPlaceholder: "微信名称",
                confirmButtonText: "确认提交"
            },
            function (inputValue) {
                if (inputValue === false) return false;
                if (inputValue === "") {
                    swal.showInputError("请输入您的微信名称!");
                    return false
                }
                $.get("<?php echo url::s('index/wechat/automaticEditName', "id=");?>" + id + "&name=" + inputValue, function (result) {
                    if (result.code == '200') {
                        swal("微信提醒", result.msg, "success");
                        setTimeout(function () {
                            location.href = '';
    ...
    function withdraw(key_id,status) {
        ...
        swal({
            ...
            function (inputValue) {
                ...
                $.get("<?php echo url::s('index/wechat/withdraw', "id=");?>" + id + "&pwd=" + inputValue +"&key_id="+key_id +"&type=wechat", function (result) {
                    if (result.code == '200') {
                        swal("微信提醒", result.msg, "success");
                        setTimeout(function () {
                            location.href = '';
    ...
    function editMaxAmount(id) {
        swal({
            ...
            function (inputValue) {
                ...
                $.get("<?php echo url::s('index/wechat/editMaxAmount', "id=");?>" + id + "&amount=" + inputValue, function (result) {
                    if (result.code == '200') {
                        swal("微信提醒", result.msg, "success");
                        setTimeout(function () {
                            location.href = '';
    ...
    function editNote(id) {
        swal({
            ...
            function (inputValue) {
                ...
                $.get("<?php echo url::s('index/wechat/editNote', "id=");?>" + id + "&note=" + inputValue, function (result) {
                    if (result.code == '200') {
                        swal("微信提醒", result.msg, "success");
                        setTimeout(function () {
                            location.href = '';
    ...
    function gatewayTest(id) {
        ...
        $('.showSweetAlert fieldset input').attr('type', 'hidden');
        $('#amount').val('1.00');
        $('#callback_url').val('https://pay.iswoole.com/index/index/callback.do');
    }
