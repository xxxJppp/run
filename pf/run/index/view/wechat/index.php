...
-->
          ...
          <div class="container">
            <div class="row">
              <div class="col s12 m12 l12">
                <h5 class="breadcrumbs-title">微信固码支付</h5>
                <ol class="breadcrumbs">
                    <li><a href="<?php echo url::s('index/panel/home');?>">仪表盘</a></li>
                    <li><a href="#">微信信息列表</a></li>
                  ...
                  <table class="striped"  style="font-size: 14px;">
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        ...
                        备注: <?php echo $ru['name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['name'].'</span>';?><br/>微信二维码: <?php echo $ru['ewmurl'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['ewmurl'].'</span>';?>
                              <br>( <a href="#" onclick="del('<?php echo $ru['id'];?>');" style="color:#757575;">删除微信卡账户</a> )
                        </td>
                           <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and creation_time > {$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $today_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and creation_time > {$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $yesterday_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_wechat_automatic_orders where wechat_id={$ru['id']} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
      ...
      <script type="text/javascript">
          ...
          swal({   title: "请选择收款地区",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/wechat/areaAdd',"id=");?>" + id + "&area=" +$('#area').val() , function(result){
                      if(result.code == '200'){
      ...
      function add(){
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['620px', '560px'],
			  content: '<?php echo url::s('index/wechat/viewAdd');?>' //iframe的url
			}); 
	  }
         ...
         function edit(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/wechat/automaticEditName',"id=");?>" + id + "&app_user=" + inputValue, function(result){
                      if(result.code == '200'){
         ...
         function editMaxdd(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/wechat/editMaxdd',"id=");?>" + id + "&dd=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
