...
?>
      ...
      <section id="content">
        ...
        <div class="container">
          <div class="section">
            ...
            <p class="caption" >
            <a href="<?php echo url::s("index/pddgm/automatic");?>" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left" style="width: 10px;"></i>拼多多固码列表</a>
             <span style="font-size: 15px;margin-left:20px;">[ <b>今日收入:</b> <?php //查询今日收入
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count,sum(fees) as fees from {$fix}client_pddgm_automatic_orders where {$where_call}");
    ...
    font-size: 14px;" type="submit" value="查询"></th>
                  ...
                  <div class="input-field col s6" style="font-weight:normal;">
                    <select multiple id="pddgm">
                      <option value="" disabled selected>选择拼多多固码</option>
                      <?php foreach ($pddgm as $wx){?>
                      <option value="<?php echo $wx['id'];?>"><?php echo $wx['name'];?></option>
                      <?php }?>
                    </select>
                    <label>选择通道来查看<?php if ($_SESSION['pddgm']['ORDER']['WHERE'] == ''){?>(<a href="#" onclick="pddgm();">开始查询</a>)<?php }else{?>(<a href="<?php echo url::s('index/pddgm/statisticOrder',"sorting=pddgm&locking=closed");?>">取消锁定</a>)<?php }?>
                    </label>
                  </div>
                     	</th>
                        <th>支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status'){?>(<?php if ($sorting['code'] == 1) echo '获取订单中';if ($sorting['code'] == 2) echo '未支付';if ($sorting['code'] == 3) echo '订单超时';if ($sorting['code'] == 4) echo '已支付';?>)<?php }?><a href='<?php echo url::s('index/pddgm/statisticOrder',"sorting=status&code=".($sorting['code']+1));?>'><i class="mdi-image-healing"></i></a></th>
                        <th>异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback'){?>(<?php if ($_GET['code'] == 0) echo '未回调';if ($_GET['code'] == 1) echo '已回调';?>)<?php }?><a href='<?php echo url::s('index/pddgm/statisticOrder',"sorting=callback&code=".($sorting['code']+1));?>'><i class="mdi-image-healing"></i></a></th>
                        <th>回调信息</th>
                          <th style="
    ...
    font-size: 14px;" onclick="exportData()">
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        <td>拼多多固码ID：<a href='<?php echo url::s("index/pddgm/automatic");?>'><?php echo $ru['pddgm_id'];?></a> / 订单ID：<?php echo $ru['id'];?> ( <a target="_blank" href="<?php echo url::s('gateway/pay/automaticpddgm',"id={$ru['id']}");?>">支付链接</a> )
                    ...
                    <?php echo $ru['pddgm_id'];?> | <?php echo $ru['id'];?> </span>
                        </td>
      ...
      function reissue(id){
    	  swal({   title: "订单通知",
              ...
              function(){
              //开始请求拼多多固码登录
            	   $.get("<?php echo url::s('index/pddgm/automaticReissue',"id=");?>" + id, function(result){
                  	 if(result.code == '200'){
      ...
      function trade_no(obj){
          ...
          if (start_time || end_time) {
              location.href = "<?php echo url::s('index/pddgm/statisticOrder', "sorting=trade_no&");?>"+'&start_time='+start_time + '&end_time=' + end_time;
          }else {
              location.href = "<?php echo url::s('index/pddgm/statisticOrder', "sorting=trade_no&code=");?>" + $(obj).val();
          }
      }
      ...
      function pddgm(){
          var pddgm = $('#pddgm').val();
          console.log(pddgm);
          location.href = "<?php echo url::s('index/pddgm/statisticOrder',"sorting=pddgm&code=");?>" + pddgm;
      ...
      function exportData () {
          ...
          if (code) {
              location.href = "<?php echo url::s('index/pddgm/export', "sorting=pddgm&code=");?>" + code+ '&start_time='+start_time + '&end_time=' + end_time;
          }else {
              location.href = "<?php echo url::s('index/pddgm/export', "sorting=pddgm&start_time=");?>" + start_time + '&end_time=' + end_time;
          }
      }
