...
-->
          ...
          <div class="container">
            <div class="row">
              <div class="col s12 m12 l12">
                <h5 class="breadcrumbs-title">拼多多固码固码支付</h5>
                <ol class="breadcrumbs">
                    <li><a href="<?php echo url::s('index/panel/home');?>">仪表盘</a></li>
                    <li><a href="#">拼多多固码信息列表</a></li>
        ...
        <div class="container">
          <div class="section">
            ...
            <p class="caption">
            <a href="<?php echo url::s("index/pddgm/automaticOrder");?>" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left" style="width: 10px;"></i>全部订单</a>
            <a onclick="add();" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-content-add left" style="width: 10px;"></i>添加拼多多固码</a>
                  ...
                  <table class="striped"  style="font-size: 14px;">
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        ...
                        备注: <?php echo $ru['name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['name'].'</span>';?><br/>拼多多固码二维码: <?php echo $ru['ewmurl'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['ewmurl'].'</span>';?>
                              <br>( <a href="#" onclick="del('<?php echo $ru['id'];?>');" style="color:#757575;">删除拼多多固码账户</a> )
                        </td>
                           <td>
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_pddgm_automatic_orders where pddgm_id={$ru['id']} and creation_time > {$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $today_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_pddgm_automatic_orders where pddgm_id={$ru['id']} and creation_time > {$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_pddgm_automatic_orders where pddgm_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $yesterday_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_pddgm_automatic_orders where pddgm_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_pddgm_automatic_orders where pddgm_id={$ru['id']} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
      ...
      <script type="text/javascript">
          ...
          swal({   title: "请选择收款地区",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/pddgm/areaAdd',"id=");?>" + id + "&area=" +$('#area').val() , function(result){
                      if(result.code == '200'){
      ...
      function add(){
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['620px', '560px'],
			  content: '<?php echo url::s('index/pddgm/viewAdd');?>' //iframe的url
			}); 
	  }
         ...
         function edit(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/pddgm/automaticEditName',"id=");?>" + id + "&app_user=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("拼多多固码提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
                          swal.showInputError(result.msg);
                      }
      ...
      function startAutomaticRb(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/pddgm/startAutomaticRb',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("拼多多固码提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
    	            	swal("拼多多固码提示", result.msg, "error");
    	              }
              	  });
            });	
      }
      ...
      function startAutomaticGateway(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/pddgm/startAutomaticGateway',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("拼多多固码提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticLogOut(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/pddgm/startAutomaticLogOut',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("拼多多固码提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function login(id){
    	  swal({   title: "拼多多固码登录",   
              ...
              function(){
              //开始请求拼多多固码登录
            	   $.get("<?php echo url::s('index/pddgm/startAutomaticLogin',"id=");?>" + id, function(result){
                  	 if(result.code == '200'){
                   		    $('.showSweetAlert p').html(result.msg);
                     		login_listen(id);
        	              }else{
        	            	swal("拼多多固码登录", result.msg, "error");
        	             }
              		});
      ...
      function login_listen(id){
      	listen_login = setInterval(function(){ $.get("<?php echo url::s('index/pddgm/getAutomaticStatus',"id=");?>" + id, function(result){
      		if(result.code > 0){	
      			if(result.code == '2' || result.code == '3' ){ $('.showSweetAlert p').html(result.msg); }
      			if(result.code == '7'){
				//将二维码展现出来扫码
      				$('.showSweetAlert h2').html('请使用拼多多固码扫一扫');
      				$('.showSweetAlert p').html("<img style='width:200px;height:200px;' src='data:image/png;base64," + result.data.img + "'/>");
      				if(music == 0){
      					play(['<?php echo FILE_CACHE . "/download/sound/拼多多固码扫一扫1.mp3";?>']);
      					music = 1;
          			}
              	}
      			if(result.code == '4'){
	            	swal("拼多多固码登录", result.msg, "success");
	              	setTimeout(function(){location.href = '';},1000);
                }
             }else{
            	 swal("拼多多固码登录", result.msg, "error");
	  ...
	  function del(id){
		  swal({   title: "拼多多固码提醒",   
             ...
             $.get("<?php echo url::s('index/pddgm/automaticDelete',"id=");?>" + id + "&pwd=" + inputValue, function(result){
              	 if(result.code == '200'){
               		    swal("拼多多固码提醒", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
	 ...
	 //轮训测试
	  function robinTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/pddgm/robinTest");?>' + '?amount=' + $('#amount').val() + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function gatewayTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/pddgm/gatewayTest");?>' + '?amount=' + $('#amount').val()  + "&keyId=" + id + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
         ...
         function editMaxAmount(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/pddgm/editMaxAmount',"id=");?>" + id + "&amount=" + inputValue, function(result){
                      if(result.code == '200'){
         ...
         function editMaxdd(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/pddgm/editMaxdd',"id=");?>" + id + "&dd=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
	  ...
	  function setting(){ 
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['600px', '400px'],
			  content: '<?php echo url::s('index/pddgm/automaticConfig');?>' //iframe的url
			}); 
	  }
