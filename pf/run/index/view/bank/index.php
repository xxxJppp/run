      ...
      <section id="content">
        ...
        <div class="container">
          <div class="section">
            ...
            <p class="caption">
            <a href="<?php echo url::s("index/bank/automaticOrder");?>" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left" style="width: 10px;"></i>全部订单</a>
            
            <a onclick="addNew();" style="font-size: 14px;background-color: red;!important;" class="btn waves-effect waves-light  cyan " ><i class="mdi-content-add left" style="width: 10px;"></i>添加银行卡</a>
                  ...
                  <table class="striped"  style="font-size: 14px;">
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                       ...
                       <br/> 卡号: <?php echo $ru['account_no'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['account_no'].'</span>';?>
                          </br>
                      账户名：<?php echo $ru['gathering_name']?> </br> 所属银行：<?php echo $ru['bank_id']?> </br>
                        备注: <?php echo $ru['name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['name'].'</span>';?>
                     ...
                     <br>( <a href="#" onclick="del('<?php echo $ru['id'];?>');" style="color:#757575;">删除银行卡账户</a> )
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_bank_automatic_orders where alipay_id={$ru['id']} and creation_time > {$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $today_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_bank_automatic_orders where alipay_id={$ru['id']} and creation_time > {$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_bank_automatic_orders where alipay_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                        $yesterday_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_bank_automatic_orders where alipay_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                            if($order[0]['count']!=0){
                        ...
                        ?><br>
                        <b>全部收入:</b> <?php 
                        $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_bank_automatic_orders where alipay_id={$ru['id']} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
      ...
      <script type="text/javascript">
          function add(){
              swal({
                  ...
                  function(){
                      $.get("<?php echo url::s('index/bank/automaticAdd');?>", function(result){
                          if(result.code == '200'){
      ...
      function addNew(){
          swal({   title: "请输入账户配置信息",
              ...
              var cardid = $('#cardid').val();
                  ...
                  $.get("<?php echo url::s('index/bank/automaticAdd',"name=");?>" +$('#name').val() +'&gathering_name='+ $('#gathering_name').val()+'&cardid='+ $('#cardid').val()+'&bank_id='+bank_id+'&account_no='+account_no, function(result){
                      if(result.code == '200'){
      ...
      function withdraw(key_id,status) {
          ...
          swal({
              ...
              function (inputValue) {
                  ...
                  $.get("<?php echo url::s('index/bank/withdraw', "id=");?>" + id + "&pwd=" + inputValue +"&key_id="+key_id +"&type=alipay", function (result) {
                      if (result.code == '200') {
          ...
          swal({   title: "请选择收款地区",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/bank/areaAdd',"id=");?>" + id + "&area=" +$('#area').val() , function(result){
                      if(result.code == '200'){
                          swal("成功提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
         ...
         function editMaxdd(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/bank/editMaxdd',"id=");?>" + id + "&dd=" + inputValue, function(result){
                      if(result.code == '200'){
      ...
      function editMaxAmount(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/bank/editMaxAmount',"id=");?>" + id + "&amount=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function editNote(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/bank/editNote',"id=");?>" + id + "&note=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
     ...
     function edit(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/bank/automaticEditName',"id=");?>" + id + "&app_user=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function startAutomaticRb(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/bank/startAutomaticRb',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("账户提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticGateway(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/bank/startAutomaticGateway',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("账户提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticLogOut(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/bank/startAutomaticLogOut',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("账户提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function login(id){
    	  swal({   title: "账户登录",   
              ...
              function(){
              //开始请求账户登录
            	   $.get("<?php echo url::s('index/bank/startAutomaticLogin',"id=");?>" + id, function(result){
                  	 if(result.code == '200'){
      ...
      function login_listen(id){
      	listen_login = setInterval(function(){ $.get("<?php echo url::s('index/bank/getAutomaticStatus',"id=");?>" + id, function(result){
      		if(result.code > 0){	
	  ...
	  function del(id){
		  swal({   title: "账户提醒",   
             ...
             $.get("<?php echo url::s('index/bank/automaticDelete',"id=");?>" + id + "&pwd=" + inputValue, function(result){
              	 if(result.code == '200'){
	 ...
	 //轮训测试
	  function robinTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/bank/robinTest");?>' + '?amount=' + $('#amount').val() + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function gatewayTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/bank/gatewayTest");?>' + '?amount=' + $('#amount').val()  + "&keyId=" + id + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function setting(){ 
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['600px', '400px'],
			  content: '<?php echo url::s('index/bank/automaticConfig');?>' //iframe的url
			}); 
	  }
