...
<section id="content">
    ...
    <div class="container">
        <div class="section">
            ...
            <p class="caption">
                <a href="<?php echo url::s("index/huafei/automatic"); ?>" style="font-size: 14px;"
                   class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left"
                                                                          style="width: 10px;"></i>话费列表</a>
                ...
                <span style="font-size: 15px;margin-left:20px;">[ <b>今日收入:</b> <?php //查询今日收入
                    ...
                    ?>
                    ] <?php if ($sorting['name'] == 'user' && $_GET['code'] != '' && $_SESSION['huafei']['WHERE'] == '') { ?> [
                        <a href="<?php echo url::s("admin/huafei/statisticOrder", "sorting=user&code={$_GET['code']}&locking=true"); ?>"
                           style="color: green;">锁定该用户查询</a> ]<?php } ?> <?php if ($_SESSION['huafei']['WHERE'] != '') { ?>  [
                        <a href="<?php echo url::s("admin/huafei/statisticOrder", "sorting=user&code={$_GET['code']}&locking=false"); ?>"
                            ...
                            <tr>
                                <th>
                                    <div class="input-field col s6" style="font-weight:normal;">
                                        <select multiple id="huafei">
                                            <option value="" disabled selected>选择话费</option>
                                            <?php foreach ($wechat as $wx) { ?>
                                                <option value="<?php echo $wx['id']; ?>"><?php echo $wx['name']; ?></option>
                                            <?php } ?>
                                        </select>
                                        <label>选择通道来查看<?php if ($_SESSION['huafei']['ORDER']['WHERE'] == '') { ?>(<a
                                                    href="#" onclick="huafei();">开始查询</a>)<?php } else { ?>(<a
                                                    href="<?php echo url::s('index/huafei/statisticOrder', "sorting=huafei&locking=closed"); ?>">取消锁定</a>)<?php } ?>
                                        </label>
                                    </div>
                                </th>
                                <th>
                                    支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status') { ?>(<?php if ($sorting['code'] == 1) echo '获取订单中';
                                        ...
                                        if ($sorting['code'] == 4) echo '已支付'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index/huafei/statisticOrder', "sorting=status&code=" . ($sorting['code'] + 1)); ?>'><i
                                ...
                                <th>
                                    异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback') { ?>(<?php if ($_GET['code'] == 0) echo '未回调';
                                        if ($_GET['code'] == 1) echo '已回调'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index/huafei/statisticOrder', "sorting=callback&code=" . ($sorting['code'] + 1)); ?>'><i
                                                class="mdi-image-healing"></i></a></th>
                                <th>回调发送次数</th>
                                <th>回调来源</th>
                                <th>回调信息</th>
    ...
    font-size: 14px;" onclick="exportData()">
                            ...
                            <?php foreach ($result['result'] as $ru) { ?>
                                <tr>
                                    <td>话费ID：<a
                                                href='<?php echo url::s("index/huafei/automatic"); ?>'><?php echo $ru['huafei_id']; ?></a>
                                        / 订单ID：<?php echo $ru['id']; ?> ( <a target="_blank"
                                                                             href="<?php echo url::s('gateway/pay/automatichuafei', "id={$ru['id']}"); ?>">支付链接</a>
                                        )
                            ...
                            <?php echo $ru['huafei_id']; ?> | <?php echo $ru['id']; ?> </span>
...
<script type="text/javascript">
    function reissue(id) {
        swal({
            ...
            function () {
                //开始请求话费登录
                $.get("<?php echo url::s('index/huafei/automaticReissue', "id=");?>" + id, function (result) {
                    if (result.code == '200') {
    ...
    function trade_no(obj) {
        ...
        if (start_time || end_time) {
            location.href = "<?php echo url::s('index/huafei/statisticOrder', "sorting=trade_no&");?>"+'&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('index/huafei/statisticOrder', "sorting=trade_no&code=");?>" + $(obj).val();
        }
    }

    function huafei() {
        var huafei = $('#huafei').val();
        location.href = "<?php echo url::s('index/huafei/statisticOrder', "sorting=huafei&code=");?>" + huafei;
    }
    ...
    function exportData () {
        ...
        if (code) {
            location.href = "<?php echo url::s('index/huafei/export', "sorting=huafei&code=");?>" + code+ '&start_time='+start_time + '&end_time=' + end_time;
        }else {
            location.href = "<?php echo url::s('index/huafei/export', "sorting=huafei&start_time=");?>" + start_time + '&end_time=' + end_time;
        }
    }
