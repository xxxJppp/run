    ...
    <!-- START CONTENT -->
      <section id="content">
        ...
        <div class="container">
          <div class="section">
            ...
            <p class="caption">
            <a href="<?php echo url::s("index/huafei/automaticOrder");?>" style="font-size: 14px;" class="btn waves-effect waves-light  cyan darken-2"><i class="mdi-editor-border-all left" style="width: 10px;"></i>全部订单</a>
         
            <a onclick="addNew();" style="font-size: 14px;background-color: red;!important;" class="btn waves-effect waves-light  cyan " ><i class="mdi-content-add left" style="width: 10px;"></i>添加话费</a>
                  ...
                  <table class="striped"  style="font-size: 14px;">
                    ...
                    <?php foreach ($result['result'] as $ru){?>
                      <tr>
                        ...
                        <td>
                        APP商户号: <?php echo $ru['app_user'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['app_user'].'</span>';?>[ <a href="#" onclick="edit('<?php echo $ru['id'];?>');">修改名称</a> ] <font color=red>( <?php if($ru['is_hongbao'] == '1'){ echo "红包模式"; }else { echo "转账模式" ;}  ?> )</font> <br>
                           备注: <?php echo $ru['name'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['name'].'</span>';?><br>
                           手机号: <?php echo $ru['phone'] == '0' ? '<span style="color:red;">Unused</span>' : '<span style="color:green;">'.$ru['phone'].'</span>';?>
                            <br>( <a href="#" onclick="del('<?php echo $ru['id'];?>');" style="color:#757575;">删除通道</a> )
                        ...
                        <td>
                          ...
                          <td>
                              ...
                              $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_huafei_automatic_orders where huafei_id={$ru['id']} and creation_time > {$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                              $today_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_huafei_automatic_orders where huafei_id={$ru['id']} and creation_time > {$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                              if($order[0]['count']!=0){
                              ...
                              $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_huafei_automatic_orders where huafei_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
                              $yesterday_order_all = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_huafei_automatic_orders where huafei_id={$ru['id']} and creation_time > {$zrTime} and creation_time<{$nowTime} and user_id={$_SESSION['MEMBER']['uid']}");
                              if($order[0]['count']!=0){
                              ...
                              ?><br>
                              <b>全部收入:</b> <?php
                              $order = $mysql->select("select sum(amount) as money,count(id) as count from {$fix}client_huafei_automatic_orders where huafei_id={$ru['id']} and status=4 and user_id={$_SESSION['MEMBER']['uid']}");
      ...
      <script type="text/javascript">
          function add(){
              swal({
                  ...
                  function(){
                      $.get("<?php echo url::s('index/huafei/automaticAdd');?>", function(result){
                          if(result.code == '200'){
                              swal("话费提示", result.msg, "success");
                              setTimeout(function(){location.href = '';},1000);
                          }else{
                              swal("话费提示", result.msg, "error");
                          }
                      });
                  });
          }
      function addNew(){
          swal({   title: "请输入话费配置信息",
                  type: "input",
                  showCancelButton: true,
                  closeOnConfirm: false,
                  html: true,
                  text: "手机号 <input type='text' id='phone' value=''>"
                  +"备注<input type='text' id='name' value=''>",
                  confirmButtonText: "提交" },
              function(inputValue){
                  if (inputValue === false) return false;
                  if (inputValue === "") {
                      swal.showInputError("参数不能为空");
                      return false
                  }
                  $.get("<?php echo url::s('index/huafei/automaticAdd',"name=");?>" +$('#name').val() +'&phone='+ $('#phone').val()+'&is_new_version=1', function(result){
                      if(result.code == '200'){
         ...
         function addhongbao(){
          swal({   title: "请输入话费配置信息",
                  ...
                  closeOnConfirm: false,
                  html: true,
                  text: "话费帐号<input type='text' id='account' value=''>"
                 +"话费PID<input type='text' id='pid' value=''>"
                  +"备注<input type='text' id='name' value=''>",
                  confirmButtonText: "提交" },
              function(inputValue){
                  if (inputValue === false) return false;
                  if (inputValue === "") {
                      swal.showInputError("话费帐号不能为空");
                      return false
                  }
                  $.get("<?php echo url::s('index/huafei/automaticAdd',"account=");?>" +$('#account').val() +'&name='+ $('#name').val() +'&pid='+ $('#pid').val()+'&is_hongbao=1', function(result){
                      if(result.code == '200'){
                          swal("成功提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function withdraw(key_id,status) {
          ...
          swal({
              ...
              function (inputValue) {
                  ...
                  $.get("<?php echo url::s('index/huafei/withdraw', "id=");?>" + id + "&pwd=" + inputValue +"&key_id="+key_id +"&type=huafei", function (result) {
                      if (result.code == '200') {
      ...
      function addArea(id){
          swal({   title: "请选择收款地区",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/huafei/areaAdd',"id=");?>" + id + "&area=" +$('#area').val() , function(result){
                      if(result.code == '200'){
                          swal("成功提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
         ...
         function editMaxdd(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/huafei/editMaxdd',"id=");?>" + id + "&dd=" + inputValue, function(result){
                      if(result.code == '200'){
      ...
      function editMaxAmount(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/huafei/editMaxAmount',"id=");?>" + id + "&amount=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function editNote(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/huafei/editNote',"id=");?>" + id + "&note=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function edit(id){
          swal({   title: "修改提醒",
              ...
              function(inputValue){
                  ...
                  $.get("<?php echo url::s('index/huafei/automaticEditName',"id=");?>" + id + "&app_user=" + inputValue, function(result){
                      if(result.code == '200'){
                          swal("提醒", result.msg, "success");
                          setTimeout(function(){location.href = '';},1000);
                      }else{
      ...
      function startAutomaticRb(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/huafei/startAutomaticRb',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("话费提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticGateway(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/huafei/startAutomaticGateway',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("话费提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function startAutomaticLogOut(id){
    	  swal({
            ...
            function(){
               $.get("<?php echo url::s('index/huafei/startAutomaticLogOut',"id=");?>" + id, function(result){
              	 if(result.code == '200'){
    	            	swal("话费提示", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
      ...
      function login(id){
    	  swal({   title: "话费登录",   
              ...
              function(){
              //开始请求话费登录
            	   $.get("<?php echo url::s('index/huafei/startAutomaticLogin',"id=");?>" + id, function(result){
                  	 if(result.code == '200'){
                   		    $('.showSweetAlert p').html(result.msg);
                     		login_listen(id);
        	              }else{
        	            	swal("话费登录", result.msg, "error");
        	             }
              		});
      ...
      function login_listen(id){
      	listen_login = setInterval(function(){ $.get("<?php echo url::s('index/huafei/getAutomaticStatus',"id=");?>" + id, function(result){
      		if(result.code > 0){	
      			if(result.code == '2' || result.code == '3' ){ $('.showSweetAlert p').html(result.msg); }
      			if(result.code == '7'){
				//将二维码展现出来扫码
      				$('.showSweetAlert h2').html('请使用话费扫一扫');
      				$('.showSweetAlert p').html("<img style='width:200px;height:200px;' src='data:image/png;base64," + result.data.img + "'/>");
      				if(music == 0){
      					play(['<?php echo FILE_CACHE . "/download/sound/话费扫一扫1.mp3";?>']);
      					music = 1;
          			}
              	}
      			if(result.code == '4'){
	            	swal("话费登录", result.msg, "success");
	              	setTimeout(function(){location.href = '';},1000);
                }
             }else{
            	 swal("话费登录", result.msg, "error");
	  ...
	  function del(id){
		  swal({   title: "话费提醒",   
             ...
             $.get("<?php echo url::s('index/huafei/automaticDelete',"id=");?>" + id + "&pwd=" + inputValue, function(result){
              	 if(result.code == '200'){
               		    swal("话费提醒", result.msg, "success");
    	              	setTimeout(function(){location.href = '';},1000);
    	              }else{
    	            	swal.showInputError(result.msg);     
    	             }
	 ...
	 //轮训测试
	  function robinTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/huafei/robinTest");?>' + '?amount=' + $('#amount').val() + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function gatewayTest(id){
		  swal({
		      ...
		      type: "prompt",
		       ...
		       window.open('<?php echo url::s("index/huafei/gatewayTest");?>' + '?amount=' + $('#amount').val()  + "&keyId=" + id + '&callback_url=' + $('#callback_url').val());
			   location.href='';
		      })
	  ...
	  function setting(){ 
		  layer.open({
			  ...
			  shade: 0.8,
			  area: ['600px', '400px'],
			  content: '<?php echo url::s('index/huafei/automaticConfig');?>' //iframe的url
			}); 
	  }
