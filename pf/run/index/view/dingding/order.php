            ...
            <p class="caption">
                ...
                <span style="font-size: 15px;margin-left:20px;">[ <b>今日收入:</b> <?php //查询今日收入
                    ...
                    ?>
                        ...
                        <a href="<?php echo url::s("admin/dingding/automaticOrder", "sorting=user&code={$_GET['code']}&locking=true"); ?>"
                           style="color: green;">锁定该用户查询</a> ]<?php } ?> <?php if ($_SESSION['ALIPAY']['WHERE'] != '') { ?>  [
                        <a href="<?php echo url::s("admin/dingding/automaticOrder", "sorting=user&code={$_GET['code']}&locking=false"); ?>"
                    ...
                    <div class="col s12 m12 l12">
                        ...
                        font-size: 14px;" type="submit" value="查询"></th>
                                ...
                                <th>
                                    ...
                                    <div class="input-field col s6" style="font-weight:normal;width:100%;">
                                        <select multiple id="alipay">
                                            <option value="" disabled selected>选择钉钉账号</option>
                                        ...
                                        <label>选择通道来查看<?php if ($_SESSION['ALIPAY']['ORDER']['WHERE'] == '') { ?>(<a
                                                    ...
                                                    href="<?php echo url::s('index/dingding/automaticOrder', "sorting=alipay&locking=closed"); ?>">取消锁定</a>)<?php } ?>
                                        </label>
                                    </div>
                                ...
                                <th>
                                    支付信息 <?php if ($sorting['code'] != 0 && $sorting['name'] == 'status') { ?>(<?php if ($sorting['code'] == 1) echo '获取订单中';
                                        ...
                                        if ($sorting['code'] == 4) echo '已支付'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index/dingding/automaticOrder', "sorting=status&code=" . ($sorting['code'] + 1)); ?>'><i
                                ...
                                <th>
                                    异步通知 <?php if ($sorting['code'] != -1 && $sorting['name'] == 'callback') { ?>(<?php if ($_GET['code'] == 0) echo '未回调';
                                        if ($_GET['code'] == 1) echo '已回调'; ?>)<?php } ?><a
                                            href='<?php echo url::s('index/dingding/automaticOrder', "sorting=callback&code=" . ($sorting['code'] + 1)); ?>'><i
                                                class="mdi-image-healing"></i></a></th>
                                <th>回调发送次数</th>
                                <th>回调来源</th>
                                <th>回调信息</th>
                            ...
                            <?php foreach ($result['result'] as $ru) { ?>
                                <tr>
                                    <td>钉钉账号ID：<a
                                                href='<?php echo url::s("index/dingding/automatic"); ?>'><?php echo $ru['dingding_id']; ?></a>
                            ...
                            <?php echo $ru['dingding_id']; ?> | <?php echo $ru['id']; ?> </span>
...
<script type="text/javascript">
    ...
    function trade_no(obj) {
        location.href = "<?php echo url::s('index/dingding/automaticOrder', "sorting=trade_no&code=");?>" + $(obj).val();
    }

    function alipay() {
        var alipay = $('#alipay').val();
        location.href = "<?php echo url::s('index/dingding/automaticOrder', "sorting=alipay&code=");?>" + alipay;
