...
class features
{
    ...
    {
        (new model())->load('user', 'group')->review('huafei_auto');
    }
    ...
    public function add($mysql)
    {
        //添加话费通道
        //检测当前拥有多少条通道
        $find_huafei_auto_count = $mysql->select("select count(id) as count from " . DB_PREFIX . "client_huafei_automatic_account where user_id={$_SESSION['MEMBER']['uid']}")[0]['count'];
        $swrc = (new model())->load('user', 'group')->check('huafei_auto');
        if (!is_array($swrc)) functions::json(-3, '您暂时还不能添加通道,请稍后再试!');
        if ($swrc['quantity'] != 0) {
            if ($find_huafei_auto_count >= $swrc['quantity']) functions::json(-2, '您当前只有' . $swrc['quantity'] . '条通道,无法再继续新增!');
        }
        $is_new_version = request::filter('get.is_new_version', 0, 'intval');
        $name = request::filter('get.name', ' ', 'htmlspecialchars');

        $phone = request::filter('get.phone', ' ', 'htmlspecialchars');
      

        $key_id = strtoupper(substr(md5(mt_rand((mt_rand(1000, 9999) + mt_rand(1000, 9999)), mt_rand(1000000, 99999999))), 0, 18));
     

        $in = $mysql->insert("client_huafei_automatic_account", [
            'name'              => $name,
            'status'            => 4,
            'login_time'        => 0,
            'heartbeats'        => 0,
        ...
        if ($in > 0) {
            ...
            functions::json(200, '恭喜您新增了一条话费Automatic通道');
        }

        functions::json(-3, '新增话费Automatic通道失败,请联系客服!');
    }
    ...
    public function startRb($mysql)
    {
        ...
        //检查该话费
        $find_wechat = $mysql->query("client_huafei_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
     //   if (!is_array($find_wechat)) functions::json(-3, '更改异常,请联系客服!');
        ...
        if ($find_wechat['training'] == 2) {
            ...
            $training = 1;
            //检测账号是否异常
         //   if ($find_wechat['status'] != 4) functions::json(-3, '更改失败,当前话费没有在线!');
        } else {
            functions::delRobin('huafei_' . $_SESSION['MEMBER']['uid'], $find_wechat['key_id']);
        }

        $update = $mysql->update("client_huafei_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function editName()
    {
        ...
        $mysql->update("client_huafei_automatic_account", $update, "id={$id}");
    ...
    //    if (!is_array($find_wechat)) functions::json(-3, '更改异常,请联系客服!');
        ...
        if( $receiving == 2 ){
            functions::delRobin('huafei_' . $find_wechat['user_id'], $find_wechat['key_id']);
        }
        $update = $mysql->update("client_huafei_automatic_account", [
            'receiving' => $receiving
        ], "id={$id}");
    ...
    public function startLogOut($mysql)
    {
        ...
        //检查该话费
        $find_wechat = $mysql->query("client_huafei_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_wechat)) functions::json(-3, '当前话费出现异常,请联系客服!');
        if ($find_wechat['status'] == 6 || $find_wechat['status'] == 1) functions::json(-3, '当前话费账号已经安全注销过了!');
        $update = $mysql->update("client_huafei_automatic_account", [
            'status' => 1
        ], "id={$id}");
    ...
    public function startLogin($mysql)
    {
        ...
        $find_wechat = $mysql->query("client_huafei_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_wechat)) functions::json(-3, '当前话费出现异常,请联系客服!');
        if ($find_wechat['status'] == 4 || $find_wechat['status'] == 6) functions::json(-3, '当前话费账号状态无法进行登录,请稍后重试!');
        $update = $mysql->update("client_huafei_automatic_account", [
            'status'     => 2,
            'login_time' => time()
        ], "id={$id}");
    ...
    public function getStatus($mysql)
    {
        ...
        if ($find_wechat['login_time'] + 120 < time() || $find_wechat['status'] == 6 || $find_wechat['status'] == 5 || $find_wechat['status'] == 1) {
            $mysql->update("client_huafei_automatic_account", ['status' => 1], "id={$id}");
            functions::json(-2, '话费登录超时!');
        }
    ...
    public function delete($mysql)
    {
        ...
        //检查该话费
        $find_wechat = $mysql->query("client_huafei_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
       // if (!is_array($find_wechat)) functions::json(-2, '删除该话费号时出现一个错误,请及时联系客服!');
      //  if ($find_wechat['status'] == 6) functions::json(-2, '当前话费正在进行安全注销,请耐心等待注销完成后再进行删除!');
       // if ($find_wechat['status'] != 1) functions::json(-2, '请将话费安全下线后再进行删除!');
        $mysql->delete("client_huafei_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
        functions::json(200, '您成功的删除了该话费!');
    }
    ...
    public function order($mysql)
    {
        ...
        //wechat
        if ($sorting == 'huafei') {
            if ($code != '' && $_SESSION['huafei']['ORDER']['WHERE'] == '') {
                $code_arr = explode(",", $code);
                if (is_array($code_arr)) {
                    $wecaht_where = '';
                    for ($i = 0; $i < count($code_arr); $i++) {
                        $wecaht_where .= ' or huafei_id=' . $code_arr[$i];
                    }

                    $_SESSION['huafei']['ORDER']['WHERE'] .= '(' . trim(trim($wecaht_where), 'or') . ')';
                }
            }

            if ($_GET['locking'] == 'closed') {
                unset($_SESSION['huafei']['ORDER']['WHERE']);
            }
        }


        $where = $where . $_SESSION['huafei']['ORDER']['WHERE'];
        ...
        //查询自己的所有话费
        $wechat = $mysql->query("client_huafei_automatic_account", "name != '0' and user_id={$_SESSION['MEMBER']['uid']}");

        $result = page::conduct('client_huafei_automatic_orders', request::filter('get.page'), 15, $where, null, 'id', 'desc');

        new view('huafei/order', [
            'result'  => $result,
            'mysql'   => $mysql,
            'sorting' => [
                'code' => $code,
  ...
  public function reissue($mysql)
    {
        ...
        $order = $mysql->query('client_huafei_automatic_orders', "id={$order_id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      ...
      //系统对代理的费率
       ...
       $shanghufees = $order['amount'] * $shanghuauthority{$module_name};
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
                ...
                if ($updateStatus !== false) {
                    ...
                    $mysql->update("client_huafei_automatic_orders", ['reached' => 1], "id={$order['id']}");
                }
            } else {
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $mysql->update("client_huafei_automatic_orders", [
                'pay_time'         => time(),
                'status'           => 4,
                'callback_time'    => time(),
                'callback_status'  => 1,
