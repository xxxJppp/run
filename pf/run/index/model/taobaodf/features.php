...
class features{
    ...
    public function add($mysql){
        //添加微信通道
        //检测当前拥有多少条通道
        $find_taobaodf_auto_count = $mysql->select("select count(id) as count from " . DB_PREFIX . "client_taobaodf_automatic_account where user_id={$_SESSION['MEMBER']['uid']}")[0]['count'];
        $swrc = (new model())->load('user', 'group')->check('taobaodf_auto');
        
        if (!is_array($swrc)) functions::json(-3, '您暂时还不能添加通道,请稍后再试!');
        if ($swrc['quantity'] != 0){
            if ($find_taobaodf_auto_count >= $swrc['quantity']) functions::json(-2, '您当前只有'.$swrc['quantity'].'条通道,无法再继续新增!');
        }
        //开始添加通道
        $key_id = strtoupper(substr(md5(mt_rand((mt_rand(1000,9999)+mt_rand(1000,9999)),mt_rand(1000000,99999999))), 0, 18));
        $in = $mysql->insert("client_taobaodf_automatic_account", [
            'name'=>0,
            'status'=>1,
            'login_time'=>0,
            'heartbeats'=>0,
   ...
   public function editName($mysql)
    ...
    public function startRb($mysql){
        $id = intval(request::filter('get.id'));
        //检查该微信
        $find_taobaodf = $mysql->query("client_taobaodf_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      //  if (!is_array($find_taobaodf)) functions::json(-3, '更改异常,请联系客服!');
        $training = 2;
        if ($find_taobaodf['training'] == 2) {
            //开启状态
            $training = 1;
            //检测账号是否异常
          //  if ($find_taobaodf['status'] != 1) functions::json(-3, '更改失败,当前微信没有在线!');
        }
        $update = $mysql->update("client_taobaodf_automatic_account", [
            'training'=>$training
        ],"id={$id}");
    ...
    public function startLogOut($mysql){
        ...
        //检查该微信
        $find_taobaodf = $mysql->query("client_taobaodf_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_taobaodf)) functions::json(-3, '当前微信出现异常,请联系客服!');
        if ($find_taobaodf['status'] == 6 || $find_taobaodf['status'] == 1) functions::json(-3, '当前微信账号已经安全注销过了!');
        $update = $mysql->update("client_taobaodf_automatic_account", [
            'status'=>6
        ],"id={$id}");
    ...
    public function startLogin($mysql){
        ...
        $find_taobaodf = $mysql->query("client_taobaodf_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_taobaodf)) functions::json(-3, '当前微信出现异常,请联系客服!');
        if ($find_taobaodf['status'] == 4 || $find_taobaodf['status'] == 6) functions::json(-3, '当前微信账号状态无法进行登录,请稍后重试!');
        $update = $mysql->update("client_taobaodf_automatic_account", [
            'status'=>2,
            'login_time'=>time()
        ],"id={$id}");
    ...
    public function getStatus($mysql){
        ...
        $find_taobaodf = $mysql->query("client_taobaodf_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        //判断微信登录时间
        if ($find_taobaodf['login_time'] + 120 < time() || $find_taobaodf['status'] == 6 || $find_taobaodf['status'] == 5 || $find_taobaodf['status'] == 1) {
            $mysql->update("client_taobaodf_automatic_account", ['status'=>1],"id={$id}");
            functions::json(-2, '微信登录超时!');
        }
        if ($find_taobaodf['status'] == 2) functions::json(2, '正在获取登录信息..');
        if ($find_taobaodf['status'] == 3) functions::json(3, '登录信息获取成功,准备登录..');
        if ($find_taobaodf['status'] == 7) functions::json(7, '请扫码登录',['img'=>$find_taobaodf['login_img']]);
        if ($find_taobaodf['status'] == 4) {
    ...
    public function delete($mysql){
        ...
        //检查该微信
        $find_taobaodf = $mysql->query("client_taobaodf_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      //  if (!is_array($find_taobaodf)) functions::json(-2, '删除该微信号时出现一个错误,请及时联系客服!');
      //  if ($find_taobaodf['status'] == 6) functions::json(-2, '当前微信正在进行安全注销,请耐心等待注销完成后再进行删除!');
       // if ($find_taobaodf['status'] != 1) functions::json(-2, '请将微信安全下线后再进行删除!');
        $mysql->delete("client_taobaodf_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
        functions::json(200, '您成功的删除了该微信!');
    }
  
   public function qrcodedelete($mysql){
        $id = intval(request::filter('get.id'));
        $pwd = functions::pwd(request::filter('get.pwd'), $_SESSION['MEMBER']['token']);
        //查询用户信息
        $pwd_server = $mysql->query("client_user","id={$_SESSION['MEMBER']['uid']}")[0]['pwd'];
        if ($pwd != $pwd_server) functions::json(-1, '您的密码输入有误!');
        //检查该微信
        $find_taobaodf = $mysql->query("client_taobaodf_qrcode","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        $mysql->delete("client_taobaodf_qrcode", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
    ...
    public function order($mysql){
        ...
        //taobaodf
        if ($sorting == 'taobaodf'){
            if ($code != '' && $_SESSION['taobaodf']['ORDER']['WHERE'] == ''){
                $code_arr = explode(",", $code);
                if (is_array($code_arr)){
                    $wecaht_where = '';
                    for ($i=0;$i<count($code_arr);$i++){
                        $wecaht_where .= ' or taobaodf_id=' . $code_arr[$i];
                    }
                    
                    $_SESSION['taobaodf']['ORDER']['WHERE'] .= '(' . trim(trim($wecaht_where),'or') . ')';
                }
            }
            
            if ($_GET['locking'] == 'closed'){
                unset($_SESSION['taobaodf']['ORDER']['WHERE']);
            }
        }
        
        
        
        $where = $where . $_SESSION['taobaodf']['ORDER']['WHERE'];
        ...
        //查询自己的所有微信
        $taobaodf = $mysql->query("client_taobaodf_automatic_account","name != '0' and user_id={$_SESSION['MEMBER']['uid']}");
        
        $result = page::conduct('client_taobaodf_automatic_orders',request::filter('get.page'),15,$where,null,'id','desc');
        
        new view('taobaodf/order',[
            'result'=>$result,
            'mysql'=>$mysql,
            'sorting'=>[
                'code'=>$code,
...
//        $end_time = request::filter('get.end_time','','htmlspecialchars');
    ...
    public function reissue($mysql)
    {
        ...
        $order = $mysql->query('client_taobaodf_automatic_orders', "id={$order_id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      ...
      //系统对代理的费率
       ...
       $shanghufees = $order['amount'] * $shanghuauthority{$module_name};
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
                ...
                if ($updateStatus !== false) {
                    ...
                    $mysql->update("client_taobaodf_automatic_orders", ['reached' => 1], "id={$order['id']}");
                }
            } else {
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $mysql->update("client_taobaodf_automatic_orders", [
                'pay_time'         => time(),
                'status'           => 4,
                'callback_time'    => time(),
                'callback_status'  => 1,
