...
class features
{
    ...
    public function add($mysql)
    {
        //添加微信通道
        //检测当前拥有多少条通道
        $find_wechat_auto_count = $mysql->select("select count(id) as count from " . DB_PREFIX . "client_wechat_automatic_account where user_id={$_SESSION['MEMBER']['uid']}")[0]['count'];
        $swrc = (new model())->load('user', 'group')->check('wechat_auto');
        if (!is_array($swrc)) functions::json(-3, '您暂时还不能添加通道,请稍后再试!');
        if ($swrc['quantity'] != 0) {
            if ($find_wechat_auto_count >= $swrc['quantity']) functions::json(-2, '您当前只有' . $swrc['quantity'] . '条通道,无法再继续新增!');
        }
        //开始添加通道
        $key_id = strtoupper(substr(md5(mt_rand((mt_rand(1000, 9999) + mt_rand(1000, 9999)), mt_rand(1000000, 99999999))), 0, 18));
        $in = $mysql->insert("client_wechat_automatic_account", [
            'name'              => 0,
            'status'            => 1,
            'login_time'        => 0,
            'heartbeats'        => 0,
    ...
    public function startRb($mysql)
    {
        ...
        //检查该微信
        $find_wechat = $mysql->query("client_wechat_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
       // if (!is_array($find_wechat)) functions::json(-3, '更改异常,请联系客服!');
        ...
        $update = $mysql->update("client_wechat_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function startGateway($mysql)
    {
      ...
      //  if (!is_array($find_wechat)) functions::json(-3, '更改异常,请联系客服!');
        ...
        if( $receiving == 2 ){
            functions::delRobin('wechat_' . $find_wechat['user_id'], $find_wechat['key_id']);
        }
        $update = $mysql->update("client_wechat_automatic_account", [
            'receiving' => $receiving
        ], "id={$id}");
    ...
    public function startLogOut($mysql)
    {
        ...
        //检查该微信
        $find_wechat = $mysql->query("client_wechat_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      //  if (!is_array($find_wechat)) functions::json(-3, '当前微信出现异常,请联系客服!');
    ...
    public function startLogin($mysql)
    {
        ...
        $find_wechat = $mysql->query("client_wechat_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_wechat)) functions::json(-3, '当前微信出现异常,请联系客服!');
        if ($find_wechat['status'] == 4 || $find_wechat['status'] == 6) functions::json(-3, '当前微信账号状态无法进行登录,请稍后重试!');
        $update = $mysql->update("client_wechat_automatic_account", [
            'status'     => 2,
            'login_time' => time()
        ], "id={$id}");
    ...
    public function getStatus($mysql)
    {
        ...
        if ($find_wechat['login_time'] + 120 < time() || $find_wechat['status'] == 6 || $find_wechat['status'] == 5 || $find_wechat['status'] == 1) {
            $mysql->update("client_wechat_automatic_account", ['status' => 1], "id={$id}");
    ...
    public function delete($mysql)
    {
        ...
        //检查该微信
        $find_wechat = $mysql->query("client_wechat_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
     //   if (!is_array($find_wechat)) functions::json(-2, '删除该微信号时出现一个错误,请及时联系客服!');
    //    if ($find_wechat['status'] == 6) functions::json(-2, '当前微信正在进行安全注销,请耐心等待注销完成后再进行删除!');
       // if ($find_wechat['status'] != 1) functions::json(-2, '请将微信安全下线后再进行删除!');
        $mysql->delete("client_wechat_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
    ...
    public function order($mysql)
    {
        ...
        //wechat
        if ($sorting == 'wechat') {
            if ($code != '' && $_SESSION['WECHAT']['ORDER']['WHERE'] == '') {
                $code_arr = explode(",", $code);
                if (is_array($code_arr)) {
                    $wecaht_where = '';
                    for ($i = 0; $i < count($code_arr); $i++) {
                        $wecaht_where .= ' or wechat_id=' . $code_arr[$i];
                    }

                    $_SESSION['WECHAT']['ORDER']['WHERE'] .= '(' . trim(trim($wecaht_where), 'or') . ')';
                }
            }

            if ($_GET['locking'] == 'closed') {
                unset($_SESSION['WECHAT']['ORDER']['WHERE']);
            }

        }

        $where = $where . $_SESSION['WECHAT']['ORDER']['WHERE'];
        ...
        //查询自己的所有微信
        $wechat = $mysql->query("client_wechat_automatic_account", "name != '0' and user_id={$_SESSION['MEMBER']['uid']}");
     ...
     public function reissue($mysql)
    {
        ...
        $order = $mysql->query('client_wechat_automatic_orders', "id={$order_id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      ...
      //系统对代理的费率
       ...
       $shanghufees = $order['amount'] * $shanghuauthority{$module_name};
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
                ...
                if ($updateStatus !== false) {
                    ...
                    $mysql->update("client_wechat_automatic_orders", ['reached' => 1], "id={$order['id']}");
                }
            } else {
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $mysql->update("client_wechat_automatic_orders", [
                'pay_time'         => time(),
                'status'           => 4,
                'callback_time'    => time(),
                'callback_status'  => 1,
