...
class features
{
    ...
    {
        (new model())->load('user', 'group')->review('shouqianba_auto');
    }
    ...
    public function add($mysql)
    {
        //添加收钱吧通道
        //检测当前拥有多少条通道
        $find_shouqianba_auto_count = $mysql->select("select count(id) as count from " . DB_PREFIX . "client_shouqianba_automatic_account where user_id={$_SESSION['MEMBER']['uid']}")[0]['count'];
        $swrc = (new model())->load('user', 'group')->check('shouqianba_auto');
        if (!is_array($swrc)) functions::json(-3, '您暂时还不能添加通道,请稍后再试!');
        if ($swrc['quantity'] != 0) {
            if ($find_shouqianba_auto_count >= $swrc['quantity']) functions::json(-2, '您当前只有' . $swrc['quantity'] . '条通道,无法再继续新增!');
        }
      ...
      $key_id = strtoupper(substr(md5(mt_rand((mt_rand(1000, 9999) + mt_rand(1000, 9999)), mt_rand(1000000, 99999999))), 0, 18));
       $type = request::filter('get.type', ' ', 'htmlspecialchars');
        ...
        $in = $mysql->insert("client_shouqianba_automatic_account", [
            'name'              => $name,
            'status'            => 4,
            'login_time'        => 0,
            'heartbeats'        => 0,
        ...
        if ($in > 0) {
            ...
            functions::json(200, '恭喜您新增了一条收钱吧Automatic通道');
        }

        functions::json(-3, '新增收钱吧Automatic通道失败,请联系客服!');
    }
    ...
    public function startRb($mysql)
    {
        ...
        //检查该收钱吧
        $find_wechat = $mysql->query("client_shouqianba_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        ...
        if ($find_wechat['training'] == 2) {
            ...
            $training = 1;
            //检测账号是否异常
            if ($find_wechat['status'] != 4) functions::json(-3, '更改失败,当前收钱吧没有在线!');
        } else {
            functions::delRobin('shouqianba_' . $_SESSION['MEMBER']['uid'], $find_wechat['key_id']);
        }

        $update = $mysql->update("client_shouqianba_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function editName()
    {
        ...
        $mysql->update("client_shouqianba_automatic_account", $update, "id={$id}");
    ...
    public function startGateway($mysql)
    {
        ...
        if( $receiving == 2 ){
            functions::delRobin('shouqianba_' . $find_wechat['user_id'], $find_wechat['key_id']);
        }
        $update = $mysql->update("client_shouqianba_automatic_account", [
            'receiving' => $receiving
        ], "id={$id}");
    ...
    public function startLogOut($mysql)
    {
        ...
        //检查该收钱吧
        $find_wechat = $mysql->query("client_shouqianba_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_wechat)) functions::json(-3, '当前收钱吧出现异常,请联系客服!');
        if ($find_wechat['status'] == 6 || $find_wechat['status'] == 1) functions::json(-3, '当前收钱吧账号已经安全注销过了!');
        $update = $mysql->update("client_shouqianba_automatic_account", [
            'status' => 1
        ], "id={$id}");
    ...
    public function startLogin($mysql)
    {
        ...
        $find_wechat = $mysql->query("client_shouqianba_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_wechat)) functions::json(-3, '当前收钱吧出现异常,请联系客服!');
        if ($find_wechat['status'] == 4 || $find_wechat['status'] == 6) functions::json(-3, '当前收钱吧账号状态无法进行登录,请稍后重试!');
        $update = $mysql->update("client_shouqianba_automatic_account", [
            'status'     => 2,
            'login_time' => time()
        ], "id={$id}");
    ...
    public function getStatus($mysql)
    {
        ...
        if ($find_wechat['login_time'] + 120 < time() || $find_wechat['status'] == 6 || $find_wechat['status'] == 5 || $find_wechat['status'] == 1) {
            $mysql->update("client_shouqianba_automatic_account", ['status' => 1], "id={$id}");
            functions::json(-2, '收钱吧登录超时!');
        }
    ...
    public function delete($mysql)
    {
        ...
        //检查该收钱吧
        $find_wechat = $mysql->query("client_shouqianba_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
       // if (!is_array($find_wechat)) functions::json(-2, '删除该收钱吧号时出现一个错误,请及时联系客服!');
      //  if ($find_wechat['status'] == 6) functions::json(-2, '当前收钱吧正在进行安全注销,请耐心等待注销完成后再进行删除!');
       // if ($find_wechat['status'] != 1) functions::json(-2, '请将收钱吧安全下线后再进行删除!');
        $mysql->delete("client_shouqianba_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
        functions::json(200, '您成功的删除了该收钱吧!');
    }
    ...
    public function order($mysql)
    {
        ...
        //wechat
        if ($sorting == 'shouqianba') {
            if ($code != '' && $_SESSION['shouqianba']['ORDER']['WHERE'] == '') {
                $code_arr = explode(",", $code);
                if (is_array($code_arr)) {
                    $wecaht_where = '';
                    for ($i = 0; $i < count($code_arr); $i++) {
                        $wecaht_where .= ' or shouqianba_id=' . $code_arr[$i];
                    }

                    $_SESSION['shouqianba']['ORDER']['WHERE'] .= '(' . trim(trim($wecaht_where), 'or') . ')';
                }
            }

            if ($_GET['locking'] == 'closed') {
                unset($_SESSION['shouqianba']['ORDER']['WHERE']);
            }
        }


        $where = $where . $_SESSION['shouqianba']['ORDER']['WHERE'];
        ...
        //查询自己的所有收钱吧
        $wechat = $mysql->query("client_shouqianba_automatic_account", "name != '0' and user_id={$_SESSION['MEMBER']['uid']}");

        $result = page::conduct('client_shouqianba_automatic_orders', request::filter('get.page'), 15, $where, null, 'id', 'desc');

        new view('shouqianba/order', [
            'result'  => $result,
            'mysql'   => $mysql,
            'sorting' => [
                'code' => $code,
...
//        $end_time = request::filter('get.end_time','','htmlspecialchars');
    ...
    {
        $module_name = 'shouqianba_auto';
        $order_id = request::filter('get.id');
        if (empty($order_id)) functions::json(-1, '订单ID错误');
        $order = $mysql->query('client_shouqianba_automatic_orders', "id={$order_id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      ...
      //系统对代理的费率
       ...
       $shanghufees = $order['amount'] * $shanghuauthority{$module_name};
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
                ...
                if ($updateStatus !== false) {
                    ...
                    $mysql->update("client_shouqianba_automatic_orders", ['reached' => 1], "id={$order['id']}");
                }
            } else {
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $mysql->update("client_shouqianba_automatic_orders", [
                'pay_time'         => time(),
                'status'           => 4,
                'callback_time'    => time(),
                'callback_status'  => 1,
