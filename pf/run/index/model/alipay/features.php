...
class features
{
    ...
    public function add($mysql)
    {
        //添加支付宝通道
        //检测当前拥有多少条通道
        $find_alipay_auto_count = $mysql->select("select count(id) as count from " . DB_PREFIX . "client_alipay_automatic_account where user_id={$_SESSION['MEMBER']['uid']}")[0]['count'];
        $swrc = (new model())->load('user', 'group')->check('alipay_auto');
        if (!is_array($swrc)) functions::json(-3, '您暂时还不能添加通道,请稍后再试!');
        if ($swrc['quantity'] != 0) {
            if ($find_alipay_auto_count >= $swrc['quantity']) functions::json(-2, '您当前只有' . $swrc['quantity'] . '条通道,无法再继续新增!');
        }
        ...
        $in = $mysql->insert("client_alipay_automatic_account", [
            'name'              => $name,
            'status'            => 4,
            'login_time'        => 0,
            'heartbeats'        => 0,
    ...
    public function startRb($mysql)
    {
        ...
        //检查该支付宝
        $find_wechat = $mysql->query("client_alipay_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
       // if (!is_array($find_wechat)) functions::json(-3, '更改异常,请联系客服!');
        ...
        if ($find_wechat['training'] == 2) {
           ...
           // if ($find_wechat['status'] != 4) functions::json(-3, '更改失败,当前支付宝没有在线!');
            ...
            functions::delRobin('alipay_' . $_SESSION['MEMBER']['uid'], $find_wechat['key_id']);
        }

        $update = $mysql->update("client_alipay_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function editName()
    {
        ...
        $mysql->update("client_alipay_automatic_account", $update, "id={$id}");
    ...
    public function startGateway($mysql)
    {
       ...
       // if (!is_array($find_wechat)) functions::json(-3, '更改异常,请联系客服!');
        ...
        if( $receiving == 2 ){
            functions::delRobin('alipay_' . $find_wechat['user_id'], $find_wechat['key_id']);
        }
        $update = $mysql->update("client_alipay_automatic_account", [
            'receiving' => $receiving
        ], "id={$id}");
    ...
    public function getStatus($mysql)
    {
        ...
        if ($find_wechat['login_time'] + 120 < time() || $find_wechat['status'] == 6 || $find_wechat['status'] == 5 || $find_wechat['status'] == 1) {
            $mysql->update("client_alipay_automatic_account", ['status' => 1], "id={$id}");
    ...
    public function delete($mysql)
    {
      ...
      //  if ($find_wechat['status'] == 6) functions::json(-2, '当前支付宝正在进行安全注销,请耐心等待注销完成后再进行删除!');
       // if ($find_wechat['status'] != 1) functions::json(-2, '请将支付宝安全下线后再进行删除!');
        $mysql->delete("client_alipay_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
    ...
    public function order($mysql)
    {
        ...
        //查询自己的所有支付宝
        $wechat = $mysql->query("client_alipay_automatic_account", "name != '0' and user_id={$_SESSION['MEMBER']['uid']}");
   ...
   public function reissue($mysql)
    {
        ...
        $order = $mysql->query('client_alipay_automatic_orders', "id={$order_id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      ...
      //系统对代理的费率
       ...
       $shanghufees = $order['amount'] * $shanghuauthority{$module_name};
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
                ...
                if ($updateStatus !== false) {
                    ...
                    $mysql->update("client_alipay_automatic_orders", ['reached' => 1], "id={$order['id']}");
                }
            } else {
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $mysql->update("client_alipay_automatic_orders", [
                'pay_time'         => time(),
                'status'           => 4,
                'callback_time'    => time(),
                'callback_status'  => 1,
