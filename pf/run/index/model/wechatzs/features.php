...
class features{
    ...
    public function __construct(){
        (new model())->load('user', 'group')->review('wechatzs_auto');
    }

    public function add($mysql){
        //添加微信通道
        //检测当前拥有多少条通道
        $find_wechatzs_auto_count = $mysql->select("select count(id) as count from " . DB_PREFIX . "client_wechatzs_automatic_account where user_id={$_SESSION['MEMBER']['uid']}")[0]['count'];
        $swrc = (new model())->load('user', 'group')->check('wechatzs_auto');
        
        if (!is_array($swrc)) functions::json(-3, '您暂时还不能添加通道,请稍后再试!');
        if ($swrc['quantity'] != 0){
            if ($find_wechatzs_auto_count >= $swrc['quantity']) functions::json(-2, '您当前只有'.$swrc['quantity'].'条通道,无法再继续新增!');
        }
        //开始添加通道
        $key_id = strtoupper(substr(md5(mt_rand((mt_rand(1000,9999)+mt_rand(1000,9999)),mt_rand(1000000,99999999))), 0, 18));
        $in = $mysql->insert("client_wechatzs_automatic_account", [
            'name'=>0,
            'status'=>1,
            'login_time'=>0,
            'heartbeats'=>0,
   ...
   public function editName($mysql)
    {
        ...
        $mysql->update("client_wechatzs_automatic_account", $update, "id={$id}");
    ...
    public function startRb($mysql){
        $id = intval(request::filter('get.id'));
        //检查该微信
        $find_wechatzs = $mysql->query("client_wechatzs_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      //  if (!is_array($find_wechatzs)) functions::json(-3, '更改异常,请联系客服!');
        $training = 2;
        if ($find_wechatzs['training'] == 2) {
            //开启状态
            $training = 1;
            //检测账号是否异常
          //  if ($find_wechatzs['status'] != 1) functions::json(-3, '更改失败,当前微信没有在线!');
        }
        $update = $mysql->update("client_wechatzs_automatic_account", [
            'training'=>$training
        ],"id={$id}");
    ...
    public function startLogOut($mysql){
        ...
        //检查该微信
        $find_wechatzs = $mysql->query("client_wechatzs_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_wechatzs)) functions::json(-3, '当前微信出现异常,请联系客服!');
        if ($find_wechatzs['status'] == 6 || $find_wechatzs['status'] == 1) functions::json(-3, '当前微信账号已经安全注销过了!');
        $update = $mysql->update("client_wechatzs_automatic_account", [
            'status'=>6
        ],"id={$id}");
    ...
    public function startLogin($mysql){
        ...
        $find_wechatzs = $mysql->query("client_wechatzs_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        if (!is_array($find_wechatzs)) functions::json(-3, '当前微信出现异常,请联系客服!');
        if ($find_wechatzs['status'] == 4 || $find_wechatzs['status'] == 6) functions::json(-3, '当前微信账号状态无法进行登录,请稍后重试!');
        $update = $mysql->update("client_wechatzs_automatic_account", [
            'status'=>2,
            'login_time'=>time()
        ],"id={$id}");
    ...
    public function getStatus($mysql){
        ...
        $find_wechatzs = $mysql->query("client_wechatzs_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        //判断微信登录时间
        if ($find_wechatzs['login_time'] + 120 < time() || $find_wechatzs['status'] == 6 || $find_wechatzs['status'] == 5 || $find_wechatzs['status'] == 1) {
            $mysql->update("client_wechatzs_automatic_account", ['status'=>1],"id={$id}");
            functions::json(-2, '微信登录超时!');
        }
        if ($find_wechatzs['status'] == 2) functions::json(2, '正在获取登录信息..');
        if ($find_wechatzs['status'] == 3) functions::json(3, '登录信息获取成功,准备登录..');
        if ($find_wechatzs['status'] == 7) functions::json(7, '请扫码登录',['img'=>$find_wechatzs['login_img']]);
        if ($find_wechatzs['status'] == 4) {
    ...
    public function delete($mysql){
        ...
        //检查该微信
        $find_wechatzs = $mysql->query("client_wechatzs_automatic_account","id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      //  if (!is_array($find_wechatzs)) functions::json(-2, '删除该微信号时出现一个错误,请及时联系客服!');
      //  if ($find_wechatzs['status'] == 6) functions::json(-2, '当前微信正在进行安全注销,请耐心等待注销完成后再进行删除!');
       // if ($find_wechatzs['status'] != 1) functions::json(-2, '请将微信安全下线后再进行删除!');
        $mysql->delete("client_wechatzs_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
    ...
    public function order($mysql){
        ...
        //wechatzs
        if ($sorting == 'wechatzs'){
            if ($code != '' && $_SESSION['wechatzs']['ORDER']['WHERE'] == ''){
                $code_arr = explode(",", $code);
                if (is_array($code_arr)){
                    $wecaht_where = '';
                    for ($i=0;$i<count($code_arr);$i++){
                        $wecaht_where .= ' or wechatzs_id=' . $code_arr[$i];
                    }
                    
                    $_SESSION['wechatzs']['ORDER']['WHERE'] .= '(' . trim(trim($wecaht_where),'or') . ')';
                }
            }
            
            if ($_GET['locking'] == 'closed'){
                unset($_SESSION['wechatzs']['ORDER']['WHERE']);
            }
        }
        
        
        
        $where = $where . $_SESSION['wechatzs']['ORDER']['WHERE'];
        ...
        //查询自己的所有微信
        $wechatzs = $mysql->query("client_wechatzs_automatic_account","name != '0' and user_id={$_SESSION['MEMBER']['uid']}");
        
        $result = page::conduct('client_wechatzs_automatic_orders',request::filter('get.page'),15,$where,null,'id','desc');
        
        new view('wechatzs/order',[
            'result'=>$result,
            'mysql'=>$mysql,
            'sorting'=>[
                'code'=>$code,
...
//        $end_time = request::filter('get.end_time','','htmlspecialchars');
   ...
   public function reissue($mysql)
    {
        $module_name = 'wechatzs_auto';
        $order_id = request::filter('get.id');
        if (empty($order_id)) functions::json(-1, '订单ID错误');
        $order = $mysql->query('client_wechatzs_automatic_orders', "id={$order_id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      ...
      //系统对代理的费率
       ...
       $shanghufees = $order['amount'] * $shanghuauthority{$module_name};
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
                ...
                if ($updateStatus !== false) {
                    ...
                    $mysql->update("client_wechatzs_automatic_orders", ['reached' => 1], "id={$order['id']}");
                }
            } else {
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $mysql->update("client_wechatzs_automatic_orders", [
                'pay_time'         => time(),
                'status'           => 4,
                'callback_time'    => time(),
                'callback_status'  => 1,
