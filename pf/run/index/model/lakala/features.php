...
class features
{
    ...
    public function add($mysql)
    {
        //添加支付宝通道
        //检测当前拥有多少条通道
        $find_lakala_auto_count = $mysql->select("select count(id) as count from " . DB_PREFIX . "client_lakala_automatic_account where user_id={$_SESSION['MEMBER']['uid']}")[0]['count'];
        $swrc = (new model())->load('user', 'group')->check('lakala_auto');
        if (!is_array($swrc)) functions::json(-3, '您暂时还不能添加通道,请稍后再试!');
        if ($swrc['quantity'] != 0) {
            if ($find_lakala_auto_count >= $swrc['quantity']) functions::json(-2, '您当前只有' . $swrc['quantity'] . '条通道,无法再继续新增!');
        }
        ...
        $in = $mysql->insert("client_lakala_automatic_account", [
            'name'              => $name,
            'status'            => 1,
    ...
    public function startRb($mysql)
    {
        ...
        //检查该支付宝
        $find_wechat = $mysql->query("client_lakala_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
        $training = 2;
        ...
        $update = $mysql->update("client_lakala_automatic_account", [
            'training' => $training
        ], "id={$id}");
    ...
    public function getStatus($mysql)
    {
        ...
        if ($find_wechat['login_time'] + 120 < time() || $find_wechat['status'] == 6 || $find_wechat['status'] == 5 || $find_wechat['status'] == 1) {
            $mysql->update("client_lakala_automatic_account", ['status' => 1], "id={$id}");
    ...
    public function delete($mysql)
    {
        ...
        $mysql->delete("client_lakala_automatic_account", "id={$id} and user_id={$_SESSION['MEMBER']['uid']}");
    ...
    public function order($mysql)
    {
        ...
        //查询自己的所有支付宝
        $wechat = $mysql->query("client_lakala_automatic_account", "name != '0' and user_id={$_SESSION['MEMBER']['uid']}");

        $result = page::conduct('client_lakala_automatic_orders', request::filter('get.page'), 15, $where, null, 'id', 'desc');

        new view('lakala/order', [
            'result'  => $result,
            'mysql'   => $mysql,
            'sorting' => [
                'code' => $code,
...
//        $end_time = request::filter('get.end_time','','htmlspecialchars');
    ...
    {
        $module_name = 'lakala_auto';
        $order_id = request::filter('get.id');
        if (empty($order_id)) functions::json(-1, '订单ID错误');
        $order = $mysql->query('client_lakala_automatic_orders', "id={$order_id} and user_id={$_SESSION['MEMBER']['uid']}")[0];
      ...
      //系统对代理的费率
       ...
       $shanghufees = $order['amount'] * $shanghuauthority{$module_name};
        ...
        if ($order['reached'] == 1) {
            ...
            if ($user_balance >= 0) {
                ...
                if ($updateStatus !== false) {
                    ...
                    $mysql->update("client_lakala_automatic_orders", ['reached' => 1], "id={$order['id']}");
                }
            } else {
        ...
        //检测订单是否为未支付
        if ($order['status'] != 4) {
            $mysql->update("client_lakala_automatic_orders", [
                'pay_time'         => time(),
                'status'           => 4,
                'callback_time'    => time(),
                'callback_status'  => 1,
    ...
    public function notOrderList($mysql){
        ...
        //查出改用户的所有银行账户
        $accountList = $mysql->query("client_lakala_automatic_account", "user_id={$_SESSION['MEMBER']['uid']}");

        if($alipay_id){
            //查出当前银行账户
            $accountListOne = $mysql->query("client_lakala_automatic_account", "id={$alipay_id}")[0];
            $notOrderList = $mysql->query("client_lakala_automatic_orders_no", "lakala_acount='{$accountListOne['key_id']}'");
            ...
            foreach ($accountList as $ak => $av) {
                $notOrderList[] = $mysql->query("client_lakala_automatic_orders_no", "lakala_acount='{$av['key_id']}'");
            }
    ...
    public function notOrderReissue($mysql)
    {
        ...
        // echo $order_id;die;
        if (empty($order_id)) functions::json(-1, '订单ID错误');
        $order = $mysql->query('client_lakala_automatic_orders_no', "id={$order_id}")[0];
        if (!is_array($order)) functions::json(-2, '当前订单不存在');
        ...
        if ($isCallback == 1) {
            ...
            $mysql->update("client_lakala_automatic_orders_no", [
                'status'         => 1,
                'pay_time'       =>$pay_time,
                'fees'       =>$fees,
            ], "id={$order['id']}");
