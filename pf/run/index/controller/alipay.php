...
class alipay
{
    ...
    {
        (new model())->load("alipay", "features")->editName($this->mysql);
    }
    ...
    function resetOrder()
    {
        ...
        foreach ($order_ids as $val) {
            ...
            $find_order = $this->mysql->query('client_alipay_automatic_orders', "id={$val}", 'id,alipay_id,status')[0];
            ...
            //查看该订单号是否属于该商户下的订单
            $account = $this->mysql->query('client_alipay_automatic_account', "id={$find_order['alipay_id']} AND user_id={$_SESSION['MEMBER']['uid']}", 'id')[0];
            if (empty($account)) {
            ...
            if (is_array($find_order)) {
                $update = $this->mysql->update("client_alipay_automatic_orders", [
                    'status'        => 4,
                    'pay_time'      => time(),
                    'callback_from' => 'index',
                ], "id={$find_order['id']}");
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_alipay_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if ($update_id > 0) {
