...
class nxys
{
    ...
    {
        (new model())->load('user', 'group')->review('nxyswx_auto');
        $result = page::conduct('client_nxys_automatic_account', request::filter('get.page'), 10, "user_id={$_SESSION['MEMBER']['uid']}", null, 'id', 'asc');
  //获取城市
    ...
    {
        (new model())->load("nxys", "features")->add($this->mysql);
}

    //启动automatic轮训
    public function startAutomaticRb()
    {
        (new model())->load("nxys", "features")->startRb($this->mysql);
    }

    //启动网关
    public function startAutomaticGateway()
    {
        (new model())->load("nxys", "features")->startGateway($this->mysql);
    }

    //修改名称
    public function automaticEditName()
    {
        (new model())->load("nxys", "features")->editName($this->mysql);
    }

    //安全注销
    public function startAutomaticLogOut()
    {
        (new model())->load("nxys", "features")->startLogOut($this->mysql);
    }

    //请求登录
    public function startAutomaticLogin()
    {
        (new model())->load("nxys", "features")->startLogin($this->mysql);
    }

    //获取拉卡拉状态
    public function getAutomaticStatus()
    {
        (new model())->load("nxys", "features")->getStatus($this->mysql);
    }

    //删除拉卡拉
    public function automaticDelete()
    {
        (new model())->load("nxys", "features")->delete($this->mysql);
    }

    //全部订单
    public function automaticOrder()
    {
        (new model())->load("nxys", "features")->order($this->mysql);
    }

    //订单统计
    public function statisticOrder()
    {
        (new model())->load("nxys", "features")->statistic($this->mysql);
    }

    //手动补发
    public function automaticReissue()
    {
        (new model())->load("nxys", "features")->reissue($this->mysql);
    }

    //轮训通道测试
    public function robinTest()
    {
        new view('nxys/robinTest');
    }

    //单个拉卡拉测试
    public function gatewayTest()
    {
        new view('nxys/gatewayTest');
    }

    //拉卡拉配置
    public function automaticConfig()
    {
        new view('nxys/setting');
    ...
    function resetOrder()
    {
        ...
        foreach ($order_ids as $val) {
            ...
            $find_order = $this->mysql->query('client_nxys_automatic_orders', "id={$val}", 'id,alipay_id,status')[0];
            ...
            //查看该订单号是否属于该商户下的订单
            $account = $this->mysql->query('client_nxys_automatic_account', "id={$find_order['alipay_id']} AND user_id={$_SESSION['MEMBER']['uid']}", 'id')[0];
            if (empty($account)) {
            ...
            if (is_array($find_order)) {
                $update = $this->mysql->update("client_nxys_automatic_orders", [
                    'status'        => 4,
                    'pay_time'      => time(),
                    'callback_from' => 'index',
                ], "id={$find_order['id']}");
                ...
                //查询用户信息
                $find_uid = $this->mysql->query("client_nxys_automatic_account", "id={$id}")[0]['user_id'];
                //写到交易记录
    ...
    public function export()
    {
        ...
        if ($start_time == 'null' && $end_time = 'null' && !$code) {
            $list = $this->mysql->query("client_nxys_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}");
        } else {
            $list = $this->mysql->query("client_nxys_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}" . $where);
        }
    ...
    public function editMaxAmount()
    {
        ...
        $mysql->update("client_nxys_automatic_account", $update, "id={$id}");
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_nxys_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if ($update_id > 0) {
    ...
    public function notOrder(){
        (new model())->load("nxys", "features")->notOrderList($this->mysql);
    }
    ...
    {
        (new model())->load("nxys", "features")->notOrderReissue($this->mysql);
    }
