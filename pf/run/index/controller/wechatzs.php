...
class wechatzs{
    ...
    public function automatic(){
        ...
        $result = page::conduct('client_wechatzs_automatic_account',request::filter('get.page'),10,"user_id={$_SESSION['MEMBER']['uid']}",null,'id','asc');
         //获取城市
    ...
    public function add(){
        ...
        //计算keyid
          $file = (new upload())->run($_FILES['ewmurl'], ROOT_PATH . '/download/cache/images/'.$_SESSION['MEMBER']['uid'], ['jpg','png','jpeg'],1000);
        ...
        //添加通道
        $rc = $this->mysql->insert("client_wechatzs_automatic_account", [
            'name'=>$name,
            'ewmurl'=>$qrcode,
            'time'=>time(),
            'status'=>4,
            'app_user' => rand(100000, 999999),
    ...
    {
        (new model())->load("wechatzs", "features")->add($this->mysql);
    }

    //启动automatic轮训
    public function startAutomaticRb()
    {
        (new model())->load("wechatzs", "features")->startRb($this->mysql);
    }

    //启动网关
    public function startAutomaticGateway()
    {
        (new model())->load("wechatzs", "features")->startGateway($this->mysql);
    }

    //安全注销
    public function startAutomaticLogOut()
    {
        (new model())->load("wechatzs", "features")->startLogOut($this->mysql);
    }

    //请求登录
    public function startAutomaticLogin()
    {
        (new model())->load("wechatzs", "features")->startLogin($this->mysql);
    }

    //获取微信状态
    public function getAutomaticStatus()
    {
        (new model())->load("wechatzs", "features")->getStatus($this->mysql);
    }

    //删除微信
    public function automaticDelete()
    {
        (new model())->load("wechatzs", "features")->delete($this->mysql);
    }

    //修改微信名称
    public function automaticEditName()
    {
        (new model())->load("wechatzs", "features")->editName($this->mysql);
    }

    //全部订单
    public function automaticOrder()
    {
        (new model())->load("wechatzs", "features")->order($this->mysql);
    }

    //订单统计
    public function statisticOrder()
    {
        (new model())->load("wechatzs", "features")->statistic($this->mysql);
    }

    //手动补发
    public function automaticReissue()
    {
        (new model())->load("wechatzs", "features")->reissue($this->mysql);
    }

    //轮训通道测试
    public function robinTest()
    {
        new view('wechatzs/robinTest');
    }

    //单个微信测试
    public function gatewayTest()
    {
        new view('wechatzs/gatewayTest');
    }

    //微信配置
    public function automaticConfig()
    {
        new view('wechatzs/setting');
    ...
    public function export()
    {
        ...
        if ($start_time == 'null' && $end_time = 'null' && !$code) {
            $list = $this->mysql->query("client_wechatzs_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}");
        } else {
            $list = $this->mysql->query("client_wechatzs_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}" . $where);
        }
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_wechatzs_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if($update_id > 0){
                \xh\library\gateway::withdraw($_SESSION['MEMBER']['uid'], $pwd, $key_id, 0, 'wechatzs');
                functions::json(200, '成功');
