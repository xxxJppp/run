...
class shouqianba
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_shouqianba_automatic_account', request::filter('get.page'), 10, "user_id={$_SESSION['MEMBER']['uid']}", null, 'id', 'asc');
        //获取城市
    ...
    {
        (new model())->load("shouqianba", "features")->add($this->mysql);
    }

    //启动automatic轮训
    public function startAutomaticRb()
    {
        (new model())->load("shouqianba", "features")->startRb($this->mysql);
    }

    //启动网关
    public function startAutomaticGateway()
    {
        (new model())->load("shouqianba", "features")->startGateway($this->mysql);
    }

    //修改名称
    public function automaticEditName()
    {
        (new model())->load("shouqianba", "features")->editName($this->mysql);
    }

    //安全注销
    public function startAutomaticLogOut()
    {
        (new model())->load("shouqianba", "features")->startLogOut($this->mysql);
    }

    //请求登录
    public function startAutomaticLogin()
    {
        (new model())->load("shouqianba", "features")->startLogin($this->mysql);
    }

    //获取收钱吧状态
    public function getAutomaticStatus()
    {
        (new model())->load("shouqianba", "features")->getStatus($this->mysql);
    }

    //删除收钱吧
    public function automaticDelete()
    {
        (new model())->load("shouqianba", "features")->delete($this->mysql);
    }

    //全部订单
    public function automaticOrder()
    {
        (new model())->load("shouqianba", "features")->order($this->mysql);
    }

    //订单统计
    public function statisticOrder()
    {
        (new model())->load("shouqianba", "features")->statistic($this->mysql);
    }

    //手动补发
    public function automaticReissue()
    {
        (new model())->load("shouqianba", "features")->reissue($this->mysql);
    }

    //轮训通道测试
    public function robinTest()
    {
        new view('shouqianba/robinTest');
    }

    //单个收钱吧测试
    public function gatewayTest()
    {
        new view('shouqianba/gatewayTest');
    }

    //收钱吧配置
    public function automaticConfig()
    {
        new view('shouqianba/setting');
    ...
    {
        unset($_SESSION['shouqianbaConfig']);
        $robin_arr = [1, 2, 3];
        $robin = intval(request::filter('get.robin'));
        if (!in_array($robin, $robin_arr)) functions::json(-1, '收钱吧配置修改失败');
        userCog::update('shouqianbaConfig', [
            'robin' => $robin
        ], $_SESSION['MEMBER']['uid']);
        functions::json(200, '收钱吧配置更新成功!');
    }
    ...
    function resetOrder()
    {
        ...
        foreach ($order_ids as $val) {
            ...
            $find_order = $this->mysql->query('client_shouqianba_automatic_orders', "id={$val}", 'id,shouqianba_id,status')[0];
            ...
            //查看该订单号是否属于该商户下的订单
            $account = $this->mysql->query('client_shouqianba_automatic_account', "id={$find_order['shouqianba_id']} AND user_id={$_SESSION['MEMBER']['uid']}", 'id')[0];
            if (empty($account)) {
            ...
            $id = $find_order['shouqianba_id'];
            $money = $find_order['money'];
            if (is_array($find_order)) {
                $update = $this->mysql->update("client_shouqianba_automatic_orders", [
                    'status'        => 4,
                    'pay_time'      => time(),
                    'callback_from' => 'index',
                ], "id={$find_order['id']}");
                ...
                //查询用户信息
                $find_uid = $this->mysql->query("client_shouqianba_automatic_account", "id={$id}")[0]['user_id'];
                //写到交易记录
                $this->mysql->insert("client_pay_record", [
                    'pay_time'     => time(),
                    'amount'       => $money,
                    'user_id'      => $find_uid,
                    'pay_note'     => '[后台补单]收钱吧ID：' . $find_order['id'] . $remark,
                    'types'        => 2,
                    'version_code' => 'shouqianba_auto',
                    'average'      => $average
                ]);
                callbacks::curl(URL_ROOT . '/server/callback/shouqianba', http_build_query(['id' => $id]));
                $successCount += 1;
                $success .= ",{$find_order['id']}";
            } else {
                $errorCount += 1;
    ...
    public function export()
    {
        ...
        if ($start_time == 'null' && $end_time = 'null' && !$code) {
            $list = $this->mysql->query("client_shouqianba_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}");
        } else {
            $list = $this->mysql->query("client_shouqianba_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}" . $where);
        }
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_shouqianba_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if ($update_id > 0) {
                \xh\library\gateway::withdraw($_SESSION['MEMBER']['uid'], $pwd, $key_id, 0, 'shouqianba');
                functions::json(200, '成功');
