...
class yunshanfu
{
    ...
    {
        (new model())->load('user', 'group')->review('yunshanfu_auto');
        $result = page::conduct('client_yunshanfu_automatic_account', request::filter('get.page'), 10, "user_id={$_SESSION['MEMBER']['uid']}", null, 'id', 'asc');
    ...
    {
        (new model())->load("yunshanfu", "features")->add($this->mysql);
}

    //启动automatic轮训
    public function startAutomaticRb()
    {
        (new model())->load("yunshanfu", "features")->startRb($this->mysql);
    }

    //启动网关
    public function startAutomaticGateway()
    {
        (new model())->load("yunshanfu", "features")->startGateway($this->mysql);
    }

    //修改名称
    public function automaticEditName()
    {
        (new model())->load("yunshanfu", "features")->editName($this->mysql);
    }

    //安全注销
    public function startAutomaticLogOut()
    {
        (new model())->load("yunshanfu", "features")->startLogOut($this->mysql);
    }

    //请求登录
    public function startAutomaticLogin()
    {
        (new model())->load("yunshanfu", "features")->startLogin($this->mysql);
    }

    //获取支付宝状态
    public function getAutomaticStatus()
    {
        (new model())->load("yunshanfu", "features")->getStatus($this->mysql);
    }

    //删除支付宝
    public function automaticDelete()
    {
        (new model())->load("yunshanfu", "features")->delete($this->mysql);
    }

    //全部订单
    public function automaticOrder()
    {
        (new model())->load("yunshanfu", "features")->order($this->mysql);
    }

    //订单统计
    public function statisticOrder()
    {
        (new model())->load("yunshanfu", "features")->statistic($this->mysql);
    }

    //手动补发
    public function automaticReissue()
    {
        (new model())->load("yunshanfu", "features")->reissue($this->mysql);
    }

    //轮训通道测试
    public function robinTest()
    {
        new view('yunshanfu/robinTest');
    }

    //单个支付宝测试
    public function gatewayTest()
    {
        new view('yunshanfu/gatewayTest');
    }

    //支付宝配置
    public function automaticConfig()
    {
        new view('yunshanfu/setting');
    ...
    function resetOrder()
    {
        ...
        foreach ($order_ids as $val) {
            ...
            $find_order = $this->mysql->query('client_yunshanfu_automatic_orders', "id={$val}", 'id,alipay_id,status')[0];
            ...
            //查看该订单号是否属于该商户下的订单
            $account = $this->mysql->query('client_yunshanfu_automatic_account', "id={$find_order['alipay_id']} AND user_id={$_SESSION['MEMBER']['uid']}", 'id')[0];
            if (empty($account)) {
            ...
            if (is_array($find_order)) {
                $update = $this->mysql->update("client_yunshanfu_automatic_orders", [
                    'status'        => 4,
                    'pay_time'      => time(),
                    'callback_from' => 'index',
                ], "id={$find_order['id']}");
                ...
                //查询用户信息
                $find_uid = $this->mysql->query("client_yunshanfu_automatic_account", "id={$id}")[0]['user_id'];
                //写到交易记录
    ...
    public function export()
    {
        ...
        if ($start_time == 'null' && $end_time = 'null' && !$code) {
            $list = $this->mysql->query("client_yunshanfu_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}");
        } else {
            $list = $this->mysql->query("client_yunshanfu_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}" . $where);
        }
    ...
    public function editMaxAmount()
    {
        ...
        $mysql->update("client_yunshanfu_automatic_account", $update, "id={$id}");
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_yunshanfu_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if ($update_id > 0) {
    ...
    public function notOrder(){
        (new model())->load("yunshanfu", "features")->notOrderList($this->mysql);
    }
    ...
    {
        (new model())->load("yunshanfu", "features")->notOrderReissue($this->mysql);
    }
