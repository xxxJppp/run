...
class wechatphone{
    ...
    public function automatic(){
        ...
        $result = page::conduct('client_wechatphone_automatic_account',request::filter('get.page'),10,"user_id={$_SESSION['MEMBER']['uid']}",null,'id','asc');
         //获取城市
	...
	$name = request::filter('get.name');
		$phone = request::filter('get.phone');
        $account = request::filter('get.account');
        //计算keyid
        $key_id = strtoupper(substr(md5(mt_rand((mt_rand(1000,9999)+mt_rand(1000,9999)),mt_rand(1000000,99999999)) . time()), 0, 18));

        //添加通道
        $rc = $this->mysql->insert("client_wechatphone_automatic_account", [
            'name'=>$name,
            'phone'=>$phone,
            'account'=>$account,
            'time'=>time(),
    ...
    {
        (new model())->load("wechatphone", "features")->add($this->mysql);
    }

    //启动automatic轮训
    public function startAutomaticRb()
    {
        (new model())->load("wechatphone", "features")->startRb($this->mysql);
    }

    //启动网关
    public function startAutomaticGateway()
    {
        (new model())->load("wechatphone", "features")->startGateway($this->mysql);
    }

    //安全注销
    public function startAutomaticLogOut()
    {
        (new model())->load("wechatphone", "features")->startLogOut($this->mysql);
    }

    //请求登录
    public function startAutomaticLogin()
    {
        (new model())->load("wechatphone", "features")->startLogin($this->mysql);
    }

    //获取微信状态
    public function getAutomaticStatus()
    {
        (new model())->load("wechatphone", "features")->getStatus($this->mysql);
    }

    //删除微信
    public function automaticDelete()
    {
        (new model())->load("wechatphone", "features")->delete($this->mysql);
    }

    //修改微信名称
    public function automaticEditName()
    {
        (new model())->load("wechatphone", "features")->editName($this->mysql);
    }

    //全部订单
    public function automaticOrder()
    {
        (new model())->load("wechatphone", "features")->order($this->mysql);
    }

    //订单统计
    public function statisticOrder()
    {
        (new model())->load("wechatphone", "features")->statistic($this->mysql);
    }

    //手动补发
    public function automaticReissue()
    {
        (new model())->load("wechatphone", "features")->reissue($this->mysql);
    }

    //轮训通道测试
    public function robinTest()
    {
        new view('wechatphone/robinTest');
    }

    //单个微信测试
    public function gatewayTest()
    {
        new view('wechatphone/gatewayTest');
    }

    //微信配置
    public function automaticConfig()
    {
        new view('wechatphone/setting');
    ...
    public function export()
    {
        ...
        if ($start_time == 'null' && $end_time = 'null' && !$code) {
            $list = $this->mysql->query("client_wechatphone_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}");
        } else {
            $list = $this->mysql->query("client_wechatphone_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}" . $where);
        }
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_wechatphone_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if($update_id > 0){
                \xh\library\gateway::withdraw($_SESSION['MEMBER']['uid'], $pwd, $key_id, 0, 'wechatphone');
                functions::json(200, '成功');
