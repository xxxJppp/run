...
class huafei{
    ...
    public function automatic(){
        ...
        $result = page::conduct('client_huafei_automatic_account',request::filter('get.page'),10,"user_id={$_SESSION['MEMBER']['uid']}",null,'id','asc');
         //获取城市
    ...
    public function add(){
        ...
        //添加通道
        $rc = $this->mysql->insert("client_huafei_automatic_account", [
            'name'=>$name,
            'ewmurl'=>$ewmurl,
            'time'=>time(),
            'status'=>4,
    ...
    {
        (new model())->load("huafei", "features")->add($this->mysql);
    }

    //启动automatic轮训
    public function startAutomaticRb()
    {
        (new model())->load("huafei", "features")->startRb($this->mysql);
    }

    //启动网关
    public function startAutomaticGateway()
    {
        (new model())->load("huafei", "features")->startGateway($this->mysql);
    }

    //安全注销
    public function startAutomaticLogOut()
    {
        (new model())->load("huafei", "features")->startLogOut($this->mysql);
    }

    //请求登录
    public function startAutomaticLogin()
    {
        (new model())->load("huafei", "features")->startLogin($this->mysql);
    }

    //获取微信状态
    public function getAutomaticStatus()
    {
        (new model())->load("huafei", "features")->getStatus($this->mysql);
    }

    //删除微信
    public function automaticDelete()
    {
        (new model())->load("huafei", "features")->delete($this->mysql);
    }

    //修改微信名称
    public function automaticEditName()
    {
        (new model())->load("huafei", "features")->editName($this->mysql);
    }

    //全部订单
    public function automaticOrder()
    {
        (new model())->load("huafei", "features")->order($this->mysql);
    }

    //订单统计
    public function statisticOrder()
    {
        (new model())->load("huafei", "features")->statistic($this->mysql);
    }

    //手动补发
    public function automaticReissue()
    {
        (new model())->load("huafei", "features")->reissue($this->mysql);
    }

    //轮训通道测试
    public function robinTest()
    {
        new view('huafei/robinTest');
    }

    //单个微信测试
    public function gatewayTest()
    {
        new view('huafei/gatewayTest');
    }

    //微信配置
    public function automaticConfig()
    {
        new view('huafei/setting');
    ...
    public function export()
    {
        ...
        if ($start_time == 'null' && $end_time = 'null' && !$code) {
            $list = $this->mysql->query("client_huafei_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}");
        } else {
            $list = $this->mysql->query("client_huafei_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}" . $where);
        }
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_huafei_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if($update_id > 0){
                \xh\library\gateway::withdraw($_SESSION['MEMBER']['uid'], $pwd, $key_id, 0, 'huafei');
                functions::json(200, '成功');
