...
class wechatbank
{
    ...
    public function automatic()
    {
        ...
        $result = page::conduct('client_wechatbank_automatic_account', request::filter('get.page'), 10, "user_id={$_SESSION['MEMBER']['uid']}", null, 'id', 'asc');
    ...
    {
        (new model())->load("wechatbank", "features")->add($this->mysql);
}

    //启动automatic轮训
    public function startAutomaticRb()
    {
        (new model())->load("wechatbank", "features")->startRb($this->mysql);
    }

    //启动网关
    public function startAutomaticGateway()
    {
        (new model())->load("wechatbank", "features")->startGateway($this->mysql);
    }

    //修改名称
    public function automaticEditName()
    {
        (new model())->load("wechatbank", "features")->editName($this->mysql);
    }

    //安全注销
    public function startAutomaticLogOut()
    {
        (new model())->load("wechatbank", "features")->startLogOut($this->mysql);
    }

    //请求登录
    public function startAutomaticLogin()
    {
        (new model())->load("wechatbank", "features")->startLogin($this->mysql);
    }

    //获取支付宝状态
    public function getAutomaticStatus()
    {
        (new model())->load("wechatbank", "features")->getStatus($this->mysql);
    }

    //删除支付宝
    public function automaticDelete()
    {
        (new model())->load("wechatbank", "features")->delete($this->mysql);
    }

    //全部订单
    public function automaticOrder()
    {
        (new model())->load("wechatbank", "features")->order($this->mysql);
    }

    //订单统计
    public function statisticOrder()
    {
        (new model())->load("wechatbank", "features")->statistic($this->mysql);
    }

    //手动补发
    public function automaticReissue()
    {
        (new model())->load("wechatbank", "features")->reissue($this->mysql);
    }

    //轮训通道测试
    public function robinTest()
    {
        new view('wechatbank/robinTest');
    }

    //单个支付宝测试
    public function gatewayTest()
    {
        new view('wechatbank/gatewayTest');
    }

    //支付宝配置
    public function automaticConfig()
    {
        new view('wechatbank/setting');
    ...
    function resetOrder()
    {
        ...
        foreach ($order_ids as $val) {
            ...
            $find_order = $this->mysql->query('client_wechatbank_automatic_orders', "id={$val}", 'id,alipay_id,status')[0];
            ...
            //查看该订单号是否属于该商户下的订单
            $account = $this->mysql->query('client_wechatbank_automatic_account', "id={$find_order['alipay_id']} AND user_id={$_SESSION['MEMBER']['uid']}", 'id')[0];
            if (empty($account)) {
            ...
            if (is_array($find_order)) {
                $update = $this->mysql->update("client_wechatbank_automatic_orders", [
                    'status'        => 4,
                    'pay_time'      => time(),
                    'callback_from' => 'index',
                ], "id={$find_order['id']}");
                ...
                //查询用户信息
                $find_uid = $this->mysql->query("client_wechatbank_automatic_account", "id={$id}")[0]['user_id'];
                //写到交易记录
    ...
    public function export()
    {
        ...
        if ($start_time == 'null' && $end_time = 'null' && !$code) {
            $list = $this->mysql->query("client_wechatbank_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}");
        } else {
            $list = $this->mysql->query("client_wechatbank_automatic_orders", "user_id={$_SESSION['MEMBER']['uid']}" . $where);
        }
   ...
   public function areaAdd()
    ...
    public function withdraw()
    {
        ...
        if ($pwd != '') {
            ...
            $mysql = new mysql();
            $update_id = $mysql->update("client_wechatbank_automatic_account", ['receiving' => 2], "key_id='{$key_id}'");
            if ($update_id > 0) {
    ...
    public function notOrder(){
        (new model())->load("wechatbank", "features")->notOrderList($this->mysql);
    }
    ...
    {
        (new model())->load("wechatbank", "features")->notOrderReissue($this->mysql);
    }
