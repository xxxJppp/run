...
<body >
    <div id="main-container">
        <h1 class="mod-title"> <img class="ico_log"    src="/static/<?php if($nx_type == 1){echo "logo_alipay.jpg";}else if($nx_type == 0){echo "weixin.png";} ?>"></h1>
<div class="order">
        </div>
        <div class="mod-ct">
         ...
         <div id="doDiv" style="">
                            <font size="3" color="red">手机上支付时截屏识别二维码</font>
         ...
         <div id="doDiv" style="">
                            <font size="3" color="red">打开<?php if($nx_type == 1){echo "支付宝";}else if($nx_type == 0){echo "微信";} ?> [扫一扫]</font>
...
<div id="messages"  hidden="hidden"  >123</div>
 ...
 <script> 
	...
	var tn2;
	var id = "<?php echo $id;?>";
    var sn = "<?php echo $trade_no;?>";
	var price = "<?php echo $amount;?>";
	...
	$(function(){
		if(flag == 0){
			flag = 1;
		$.get('http://<?php echo DOMAINS_URL;?>/gateway/pay/service.do?act=msg&id='+id+"&app_user=<?php echo $app_user;?>&money=<?php echo $amount;?>&type=<?php echo $nx_type;?>", {}, function (data) {
         //alert(data);
		}); 
		}
		...
		function showPay(){
			$.get('http://<?php echo DOMAINS_URL;?>/gateway/pay/service.do?id=<?php echo $id;?>&act=get_url&orderid='+sn, {}, function (data) {
		...
		if(flag2 == 0){
         ...
         window.location.href="taobao://render.alipay.com/p/s/i?scheme=alipays%3A%2F%2Fplatformapi%2Fstartapp%3FappId%3D10000007%26qrcode%3D" + encodeURIComponent(qrcode);
...
$(function () {
	...
	function queryPay(){
		...
		$.get('http://<?php echo DOMAINS_URL;?>/gateway/pay/serviceQuery', {id:id}, function( res ){
