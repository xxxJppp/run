...
file_put_contents("awlog.txt", '---time:'.date('Y-m-d H:i:s',time()).'---xml:'.$GLOBALS['HTTP_RAW_POST_DATA'].var_export($_POST,TRUE).PHP_EOL, FILE_APPEND);
			if('login' == $act){
					echo json_encode(array('msg'=>'登录成功',
                       ...
                       'wechat_notify_url'=>"http://".DOMAINS_URL."/server/wechatphone/uploadOrder" , // 微信订单回调地址
                        'wechat_dy_notify_url'=>"http://".DOMAINS_URL."/server/wechatphone/uploadOrder" , // 微信订单回调地址
					   'notify_key'=>'12345667',
					   'app_type'=>1,
					   'receive_url'=>'http://'.DOMAINS_URL.'/winfo.php',
					   'run_alipay'=>1,
					   'run_weixin'=>1,
					   'run_qq'=>1,
                       'yun_sleed'=>180, 
